"""Replay-visible governed runtime execution commit evidence."""
...
def runtime_execution_commit_evidence(*, binding: dict, valid: bool, states: tuple[str, ...]) -> dict:
    return {
        **{field: binding.get(field, "") for field in FIELDS},
        "activation_authorized": binding.get("activation_authorized") is True,
        "bounded_runtime_committed": bool(binding.get("bounded_runtime_id")),
        "result_captured": bool(binding.get("result_capture_id")),
        "response_returned": bool(binding.get("response_return_id")),
        "states": list(states),
        "replay_safe": valid,
