...
from .live_codex_validation_case import (
    VALIDATION_NAME,
    VALIDATION_NAME_V2,
...
from .live_codex_validation_evidence import (
    live_codex_validation_evidence,
    live_codex_validation_evidence_v2,
...
def run_live_codex_validation_v2(
    *,
    workspace_path: str | Path,
    codex_executable: str | None = None,
    timeout_seconds: int = 30,
    ...
    detected = bool(codex_executable or detect_codex_cli())
    case = create_live_codex_validation_case_v2(
        workspace_path=workspace_path,
        codex_executable=codex_executable,
        timeout_seconds=timeout_seconds,
    )
    if isinstance(case, dict) and case.get("status") == "BLOCKED":
        evidence = live_codex_validation_evidence_v2(
            status="BLOCKED",
            codex_cli_detected=False,
            case=None,
            blocked_reason=case["blocked_reason"],
    ...
    if execution["bounded_execution_status"] == "SUCCESS":
        ...
        status = "BLOCKED"
        blocked_reason = f"live Codex exec did not complete successfully: {execution['bounded_execution_status']}"
    evidence = live_codex_validation_evidence_v2(
        status=status,
        codex_cli_detected=detected,
        case=case_dict,
