"""Governed runtime execution surface policy."""

from sapianta_bridge.human_interaction_continuity.interaction_session import stable_hash


def create_runtime_execution_surface_policy(*, runtime_execution_surface_id: str) -> dict:
    value = {"runtime_execution_surface_id": runtime_execution_surface_id}
    return {
        "runtime_execution_surface_policy_id": f"RUNTIME-EXECUTION-SURFACE-POLICY-{stable_hash(value)[:24]}",
        **value,
        "dynamic_surface_inference_allowed": False,
        "shell_true_allowed": False,
        "raw_shell_execution_allowed": False,
        "unrestricted_subprocess_allowed": False,
        "unrestricted_network_execution_allowed": False,
    }
