...
def test_conversational_routing_records_coverage(tmp_path) -> None:
    ...
    assert coverage["registered_workflows"] == 20
    assert coverage["conversationally_accessible_workflows"] == 20
    assert coverage["coverage_ratio"] == "20/20"
...
def test_conversational_route_cli_renders_selection(tmp_path) -> None:
    ...
    assert "coverage: 20/20" in rendered
