from sapianta_bridge.governed_runtime_activation_gate.runtime_activation_gate_validator import validate_runtime_activation_gate
...
def test_runtime_activation_gate_validator_rejects_unauthorized_activation():
    result = validate_runtime_activation_gate(
        gate_session={"runtime_activation_gate_id": "ACT-1", "local_runtime_bridge_session_id": "BR-1", "activation_authorized": False, "approved_by": "human"},
        binding=_binding(),
        bridge_output={"validation": {"valid": True}, "local_runtime_bridge_binding": {"local_runtime_bridge_session_id": "BR-1"}},
    )
    assert result["valid"] is False
    assert any(error["field"] == "activation_authorized" for error in result["errors"])
