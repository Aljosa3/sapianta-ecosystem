...
def _commit_output(*, authorized=True):
    ...
    activation = create_runtime_activation_gate(bridge_output=bridge, activation_authorized=authorized, approved_by="human")
    return create_runtime_execution_commit(activation_output=activation)


def test_runtime_delivery_finalization_closes_valid_commit():
    result = create_runtime_delivery_finalization(commit_output=_commit_output())
    assert result["validation"]["valid"] is True
    assert result["states"][-1] == "RUNTIME_DELIVERY_FINALIZATION_CLOSED"


def test_runtime_delivery_finalization_rejects_unapproved_commit():
    result = create_runtime_delivery_finalization(commit_output=_commit_output(authorized=False))
    assert result["validation"]["valid"] is False
    assert result["states"] == ["RUNTIME_DELIVERY_FINALIZATION_REJECTED"]


def test_runtime_delivery_finalization_blocks_incomplete_commit():
    result = create_runtime_delivery_finalization(commit_output={})
    assert result["states"] == ["RUNTIME_DELIVERY_FINALIZATION_BLOCKED"]
