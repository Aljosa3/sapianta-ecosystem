...
def _serving_output():
    ...
    turn = run_interaction_loop_turn(
        "Inspect governance evidence",
        session=loop,
        turn_index=1,
        execution_gate_id="G-1",
    ...
    return attach_runtime_serving_turn(serving_session=serving, session_runtime_output=session_output)


def test_terminal_attachment_controller_emits_bounded_response():
    result = attach_governed_terminal_runtime(runtime_serving_output=_serving_output(), stdin_binding_id="STDIN-1", stdout_binding_id="STDOUT-1")
    assert result["validation"]["valid"] is True
    assert result["states"][-1] == "TERMINAL_RESPONSE_EMITTED"
    assert result["terminal_attachment_binding"]["runtime_serving_session_id"]


def test_terminal_attachment_controller_blocks_incomplete_runtime_linkage():
    result = attach_governed_terminal_runtime(runtime_serving_output={}, stdin_binding_id="STDIN-1", stdout_binding_id="STDOUT-1")
    assert result["states"] == ["BLOCKED"]


def test_terminal_attachment_controller_blocks_identity_change():
    first = attach_governed_terminal_runtime(runtime_serving_output=_serving_output(), stdin_binding_id="STDIN-1", stdout_binding_id="STDOUT-1")
    second = attach_governed_terminal_runtime(runtime_serving_output=_serving_output(), stdin_binding_id="STDIN-2", stdout_binding_id="STDOUT-1", prior_output=first)
