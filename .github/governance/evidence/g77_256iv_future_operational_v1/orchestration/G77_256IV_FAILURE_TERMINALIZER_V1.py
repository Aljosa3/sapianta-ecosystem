#!/usr/bin/env python3
"""Seal the consumed G77-256IV pre-request guest-import failure and teardown.
...
TRANSIENT = Path("/tmp/g77_256iv_future_operational_v1")
SERIAL_SOURCE = TRANSIENT / "serial.log"
SERIAL = IV / "G77_256IV_SERIAL_CONSOLE_V1.log"
PRE = RECEIPTS / "G77_256IV_PRE_EXECUTED_QEMU_ARGV_RECEIPT_V1.json"
POST = RECEIPTS / "G77_256IV_POST_EXECUTED_QEMU_ARGV_RECEIPT_V1.json"
RAW = RUNTIME / "G77_256IV_RAW_EXECUTION_EVIDENCE_V1.jsonl"
GUEST_EXECUTION = RUNTIME / "G77_256IV_GUEST_EXECUTION_SEAL_V1.json"
GUEST_TEARDOWN = RUNTIME / "G77_256IV_GUEST_TEARDOWN_SEAL_V1.json"
AUTHORITY = IV / "G77_256IV_FRESH_HUMAN_OPERATIONAL_AUTHORIZATION_HANDOFF_V1.json"
CONSUMPTION = IV / "G77_256IV_AUTHORITY_VALIDATION_AND_CONSUMPTION_CHECKPOINT_V1.json"
CONTEXT = IV / "live_binding/SAPIANTA_FRESH_OPERATION_CONTEXT_V1.json"
INDEPENDENT = IV / "G77_256IV_INDEPENDENT_TERMINAL_EVIDENCE_REDUCTION_V1.json"
PRE_TEARDOWN = IV / "G77_256IV_SPCE_HOST_PRE_TEARDOWN_CHECKPOINT_V1.json"
TEARDOWN = IV / "G77_256IV_SPCE_HOST_TEARDOWN_CHECKPOINT_V1.json"
FINAL_SEAL = IV / "G77_256IV_SPCE_FINAL_EXECUTION_SEAL_V1.json"
...
LAST_VERIFIED = (
    ...
    "FUTURE_GUEST_ADAPTER_PROCESS_STARTED"
)
FIRST_BROKEN = (
    "FUTURE_GUEST_ADAPTER_TOP_LEVEL_AIGOL_IMPORT_BEFORE_"
    "GUEST_CHECKOUT_IMPORT_ROOT_BINDING"
)
...
MINIMUM_MISSING = (
    "GUEST_BOOTSTRAP_IMPORT_ROOT_BINDING_FOR_AIGOL_BEFORE_"
    "FUTURE_ADAPTER_TOP_LEVEL_IMPORTS"
)
MINIMUM_NEXT = (
    "SEPARATE_HUMAN_REVIEWED_REPOSITORY_ONLY_SUCCESSOR_GENERATION_TO_BIND_AND_"
    "STATICALLY_VERIFY_PYTHON_IMPORT_ROOT__NO_RETRY_IN_IV"
)
...
def main() -> int:
    ...
    if any(path.exists() or path.is_symlink() for path in targets):
        raise RuntimeError("IV failure terminal namespace is not fresh")
    ...
    if not qemu_absent():
        raise RuntimeError("IV QEMU process still present")
    ...
    if any(path.exists() or path.is_symlink() for path in (RAW, GUEST_EXECUTION, GUEST_TEARDOWN)):
        raise RuntimeError("unexpected guest operational evidence after import failure")

    serial = SERIAL_SOURCE.read_bytes()
    required = (
        b"G77_256FM_BOOT_MARKER=PASS",
        b'File "/mnt/dp-harness/G77_256FM_WRONG_ATTEMPT_VECTOR_ADAPTER_V1.py", line 20',
        b"from aigol.runtime.canonical_che_evidence_correlation_contract_v1 import",
        b"ModuleNotFoundError: No module named 'aigol'",
        b"G77_256FM_HARNESS_EXIT_STATUS=1",
    )
    if any(token not in serial for token in required):
        raise RuntimeError("decisive guest import failure evidence incomplete")
    locations = {
    ...
    copy_serial()
    pre_checkpoint = {
        "schema_id": "G77_256IV_SPCE_HOST_PRE_TEARDOWN_CHECKPOINT_V1",
        ...
        "failure_observation": {
            ...
            "exception_reason": "No module named 'aigol'",
    ...
    write_json(
        PRE_TEARDOWN,
        seal("G77_256IV_SPCE_HOST_PRE_TEARDOWN_CHECKPOINT_ENVELOPE_V1", "checkpoint", pre_checkpoint),
    )

    if TRANSIENT.resolve() != Path("/tmp/g77_256iv_future_operational_v1"):
    ...
    teardown = {
        "schema_id": "G77_256IV_SPCE_HOST_TEARDOWN_CHECKPOINT_V1",
    ...
    write_json(
        TEARDOWN,
        seal("G77_256IV_SPCE_HOST_TEARDOWN_CHECKPOINT_ENVELOPE_V1", "checkpoint", teardown),
    )

    reduction = {
        "schema_id": "G77_256IV_INDEPENDENT_TERMINAL_EVIDENCE_REDUCTION_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        "terminal": TERMINAL_CLASS,
        "observed_terminal_shape": "GUEST_TOP_LEVEL_IMPORT_FAILURE_BEFORE_FUTURE_REQUEST",
        "failure": {
            "exception_type": "ModuleNotFoundError",
            "exception_reason": "No module named 'aigol'",
            "exact_failure": EXACT_FAILURE,
        ...
        "future_semantics": {
            ...
            "semantic_result": "NOT_REACHED_OPERATIONALLY__IMPORT_FAILED_BEFORE_REQUEST",
        },
    ...
    write_json(
        INDEPENDENT,
        seal("G77_256IV_INDEPENDENT_TERMINAL_EVIDENCE_REDUCTION_ENVELOPE_V1", "reduction", reduction),
    )

    artifacts = {
        path.relative_to(IV).as_posix(): sha256(path)
        for path in (
    ...
    final = {
        "schema_id": "G77_256IV_SPCE_FINAL_EXECUTION_SEAL_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        "terminal": TERMINAL_CLASS,
    ...
    write_json(
        FINAL_SEAL,
        seal("G77_256IV_SPCE_FINAL_EXECUTION_SEAL_ENVELOPE_V1", "seal", final),
    )

    terminal = {
        "schema_id": "G77_256IV_SPCE_TERMINAL_REDUCTION_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        "terminal": TERMINAL_CLASS,
    ...
    write_json(
        TERMINAL,
        seal("G77_256IV_SPCE_TERMINAL_REDUCTION_ENVELOPE_V1", "reduction", terminal),
    )
    print(json.dumps({
        "terminal": TERMINAL_CLASS,
        "verdict": VERDICT,
