#!/usr/bin/env python3
"""Materialize the nonauthority G77-256IV FUTURE Human barrier.

The only executable path authenticates committed IU, reuses the certified IF
...
IV_ROOT = ROOT / ".github/governance/evidence/g77_256iv_future_operational_v1"
LIVE = IV_ROOT / "live_binding"
OPERATION_ROOT = IV_ROOT / "operation_state"
TRANSIENT_ROOT = Path("/tmp/g77_256iv_future_operational_v1")

BRANCH = "g77-256fl-wrong-attempt-preboot-blocker"
ORIGIN = "git@github.com:Aljosa3/sapianta-ecosystem.git"
HEAD = "30bb9fd8d983e56362d7c95fd5d15581da27f703"
TREE = "1e9d8b76771b3cdb94e13db80bac3b097395fcf4"
SUBJECT = "G77-256IU certify FUTURE post-commit static readiness"
...
IU_ROOT = Path(".github/governance/evidence/g77_256iu_post_commit_future_static_readiness_v1")
IU_REPORT = IU_ROOT / "G77_256IU_G48_POST_COMMIT_STATIC_READINESS_REPORT_V1.md"
IU_TERMINAL = IU_ROOT / "G77_256IU_SPCE_TERMINAL_REPOSITORY_ONLY_REDUCTION_V1.json"
IU_FORMALIZER = IU_ROOT / "analysis/G77_256IU_POST_COMMIT_FUTURE_STATIC_READINESS_FORMALIZER_V1.py"
IU_TEST = IU_ROOT / "tests/test_g77_256iu_post_commit_future_static_readiness_v1.py"
IU_HASHES = {
    IU_REPORT: "212ebcc177741c419b46809768c3b7dd9d4a09a89eff840640b69da57048d76a",
    IU_TERMINAL: "41c53cfb8edb8e71b13f535c720950e2d09f8659f2aeefb918bbd9e8e6ed3d68",
    IU_FORMALIZER: "4a20e7677bb9076292666d4b7250ed06e6d1726f9251f962b37f01235f638a02",
    IU_TEST: "d7ffc2e67ec2d9bd4620ed7ed19faad68d729e02c8fc8eb3c01d92eae182459d",
}
...
class IVBarrierError(ValueError):
    """One deterministic fail-closed IV barrier error."""
...
def load_module(relative: Path, name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise IVBarrierError(f"MODULE_UNAVAILABLE:{relative}")
...
FM = load_module(FM_PATH, "g77_256iv_fm")
GN = load_module(GN_PATH, "g77_256iv_gn")
GL = load_module(GL_PATH, "g77_256iv_gl")
DU = load_module(DU_PATH, "g77_256iv_du_v2")
EB = load_module(EB_PATH, "g77_256iv_eb_v2")
EE = load_module(EE_PATH, "g77_256iv_ee_v2")
...
def unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, item in pairs:
        if key in value:
            raise IVBarrierError(f"DUPLICATE_KEY:{key}")
        value[key] = item
    return value
...
def load_canonical(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or raw != canonical_bytes(value):
        raise IVBarrierError(f"NONCANONICAL_JSON:{path}")
    return value
...
def write_json(path: Path, value: dict[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        raise IVBarrierError(f"FRESH_ARTIFACT_COLLISION:{path}")
...
def authenticate_entry(remote_head: str, nested_remote_tag: str) -> dict[str, Any]:
    ...
    if any(observed[key] != expected for key, expected in expected.items()):
        raise IVBarrierError("EXACT_COMMITTED_IU_ENTRY_MISMATCH")
    ...
    for label, revision in LINEAGE.items():
        if subprocess.run(
            ...
            raise IVBarrierError(f"LINEAGE_MISSING:{label}")
    ...
    if nested_state != {
        ...
        raise IVBarrierError("NESTED_AUTHORITY_MISMATCH")
    return observed | {"local_remote_equality": "VERIFIED", "lineage": lineage, "nested_authority": nested_state}


def reconstruct_iu() -> dict[str, Any]:
    identities: dict[str, Any] = {}
    for relative, expected_sha in IU_HASHES.items():
        committed = subprocess.check_output(["git", "show", f"{HEAD}:{relative}"], cwd=ROOT)
        current = (ROOT / relative).read_bytes()
        if committed != current or sha256_bytes(committed) != expected_sha:
            raise IVBarrierError(f"IU_COMMITTED_BYTE_MISMATCH:{relative}")
        identities[relative.name] = {
            "git_blob": git("rev-parse", f"{HEAD}:{relative}"),
            "sha256": expected_sha,
        }
    envelope = load_canonical(ROOT / IU_TERMINAL)
    if envelope["reduction_sha256"] != sha256_bytes(canonical_bytes(envelope["reduction"])):
        raise IVBarrierError("IU_INNER_SEAL_MISMATCH")
    reduction = envelope["reduction"]
    if reduction["terminal"] != "A__FUTURE_POST_COMMIT_FULL_STATIC_READINESS_VERIFIED":
        raise IVBarrierError("IU_TERMINAL_MISMATCH")
    required = {
    ...
    binding = reduction["bootstrap_seed_selector"]
    if any(binding[key] != value for key, value in required.items()):
        raise IVBarrierError("IU_BOOTSTRAP_SEED_SELECTOR_RECONSTRUCTION_MISMATCH")
    readiness = reduction["readiness"]
    for key in (
        "du_v2_post_commit_readiness", "eb_v2_post_commit_readiness",
        "ee_v2_post_commit_readiness", "du_eb_ee_v2_chain",
        "fm_authority_free_static_readiness",
        "future_post_commit_preoperational_readiness",
        "existing_fm_guest_consumer_bound",
    ):
        if readiness[key] != "VERIFIED":
            raise IVBarrierError(f"IU_READINESS_RECONSTRUCTION_MISMATCH:{key}")
    if (
        reduction["route_firewall"]["parallel_flow_created"] != "VERIFIED__NO"
        or reduction["route_firewall"]["production_route_delta"] != "VERIFIED__0"
        or reduction["boundary_firewalls"]["p11_mutation_count"] != "VERIFIED__0"
        or reduction["operational_counters"]["e05"] != "VERIFIED__10_OF_18"
    ):
        raise IVBarrierError("IU_TERMINAL_BOUNDARY_MISMATCH")
    return {
        ...
        "ex_reused": reduction["boundary_firewalls"]["ex_reused"],
        "ex_reconstructed": reduction["boundary_firewalls"]["ex_reconstructed"],
    }
...
def reconstruct_ir() -> dict[str, Any]:
    ...
    for relative, expected_sha in IR_HASHES.items():
        ...
        current = (ROOT / relative).read_bytes()
        if committed != current or sha256_bytes(committed) != expected_sha:
            raise IVBarrierError(f"IR_COMMITTED_BYTE_MISMATCH:{relative}")
        identities[relative.name] = {
            "git_blob": git("rev-parse", f"{HEAD}:{relative}"),
            "sha256": expected_sha,
        }
...
def authenticate_future_semantics() -> dict[str, Any]:
    ...
    if envelope["binding_sha256"] != sha256_bytes(canonical_bytes(binding)):
        raise IVBarrierError("IF_ACT_CHE_SEAL_MISMATCH")
    ...
    if not result["evaluation"] < result["valid_from"] < result["valid_until"]:
        raise IVBarrierError("FUTURE_TIME_RELATION_MISMATCH")
    if (result["payload_digest"], result["source_act"], result["che_correlation"]) != (
        FUTURE_PAYLOAD, FUTURE_SOURCE_ACT, FUTURE_CHE,
    ):
        raise IVBarrierError("FUTURE_IDENTITY_MISMATCH")
    result.update({"future_semantic_mutation_count": 0, "wall_clock_dependency_count": 0})
    return result


def derive_identity() -> dict[str, Any]:
    expected_root = f"g77_256{PREFIX[-2:].lower()}_{VECTOR.lower()}_operational_v1"
    if IV_ROOT.name != expected_root:
        raise IVBarrierError("EVIDENCE_NAMESPACE_IDENTITY_MISMATCH")
    if FM.fresh_context.operation_vector(GENERATION) != VECTOR:
        raise IVBarrierError("CLOSED_VECTOR_IDENTITY_DERIVATION_FAILED")
    for identity in (GENERATION, OPERATION):
        ...
        if result.returncode not in (0, 1) or result.stdout:
            raise IVBarrierError(f"COMMITTED_IDENTITY_COLLISION:{identity}")
    return {
...
def materialize_v2_binding() -> dict[str, Any]:
    ...
    if sha256_path(source) != FUTURE_CANDIDATE_SHA:
        raise IVBarrierError("IF_CANDIDATE_IDENTITY_MISMATCH")
    ...
    readiness_candidate = readiness_root / "candidate/G77_256IV_V2_READINESS_CANDIDATE_V2.json"
    ...
    harness = bindings / "G77_256IV_EE_PATH_PROJECTION_FIXTURE_V1.py"
    eb_path = bindings / "G77_256IV_EB_RECEIPT_V2.json"
    ee_path = bindings / "G77_256IV_EE_RECEIPT_V2.json"
    for parent in (
    ...
    if set(du_result.values()) != {"PASS"} or eb_result["overall_result"] != "PASS":
        raise IVBarrierError("DU_EB_V2_BINDING_FAILED")
    if ee_result["pre_materialization_runtime_path_binding_result"] != "PASS":
        raise IVBarrierError("EE_V2_BINDING_FAILED")
    ...
    if (target["head"], target["tree"]) != (IF_HEAD, IF_TREE) or baseline != {"head": HEAD, "tree": TREE}:
        raise IVBarrierError("V2_RUNTIME_CERTIFICATION_ROLE_BINDING_FAILED")
    if ee["receipt"]["runtime_target_selection_binding"] != target or ee["receipt"]["certification_baseline"] != baseline:
        raise IVBarrierError("EB_EE_ROLE_DISAGREEMENT")
    return {
...
def materialize(args: argparse.Namespace) -> None:
    if LIVE.exists() or OPERATION_ROOT.exists() or TRANSIENT_ROOT.exists():
        raise IVBarrierError("IV_ONE_SHOT_NAMESPACE_NOT_FRESH")
    ...
    static = sealed(
        "G77_256IV_PREAUTHORITY_STATIC_READINESS_ENVELOPE_V1", "proof",
        {
            "schema_id": "G77_256IV_PREAUTHORITY_STATIC_READINESS_V1",
            "recorded_at_utc": recorded, "entry": entry,
            "iu_reconstruction": iu, "ir_reconstruction": ir,
    ...
    static_path = IV_ROOT / "G77_256IV_PREAUTHORITY_STATIC_READINESS_V1.json"
    write_json(static_path, static)

    observation = GL.prepare_and_observe_receipt_parent(ROOT, context)
    observation_path = IV_ROOT / "G77_256IV_GL_RECEIPT_PARENT_OBSERVATION_V1.json"
    ...
    equivalence = sealed(
        "G77_256IV_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256IV_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION, **gl_result},
    )
    equivalence_path = IV_ROOT / "G77_256IV_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1.json"
    write_json(equivalence_path, equivalence)

    counters = zero_counters()
    checkpoint_inner = {
        "schema_id": "G77_256IV_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1",
        ...
        "iu_terminal_reconstruction": iu, "ir_terminal_reconstruction": ir,
    ...
    checkpoint = sealed(
        "G77_256IV_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_ENVELOPE_V1",
        "checkpoint", checkpoint_inner,
    )
    checkpoint_path = IV_ROOT / "G77_256IV_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1.json"
    write_json(checkpoint_path, checkpoint)

    request_inner = {
        "schema_id": "G77_256IV_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1",
    ...
    request = sealed("G77_256IV_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_ENVELOPE_V1", "request", request_inner)
    request_path = IV_ROOT / "G77_256IV_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1.json"
    write_json(request_path, request)
    presentation = GN.render_human_authorization_presentation(request_path)
    presentation_path = IV_ROOT / "G77_256IV_HUMAN_OPERATIONAL_AUTHORIZATION_PRESENTATION_V1.txt"
    if presentation_path.exists() or presentation_path.is_symlink():
        raise IVBarrierError("FRESH_PRESENTATION_COLLISION")
    ...
    gn_proof = sealed(
        "G77_256IV_GN_HUMAN_PRESENTATION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256IV_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION,
    ...
    write_json(IV_ROOT / "G77_256IV_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1.json", gn_proof)

    counters["authorization_presentation"] = 1
    reduction = sealed(
        "G77_256IV_PREHUMAN_PHASE_A_REDUCTION_ENVELOPE_V1", "reduction",
        {
            "schema_id": "G77_256IV_PREHUMAN_PHASE_A_REDUCTION_V1",
            "recorded_at_utc": recorded, "generation_identity": GENERATION,
            "operation_identity": OPERATION,
            "terminal": "HUMAN_AUTHORIZATION_REQUIRED",
            "entry": entry, "iu_reconstruction": iu,
    ...
    write_json(IV_ROOT / "G77_256IV_PREHUMAN_PHASE_A_REDUCTION_V1.json", reduction)
