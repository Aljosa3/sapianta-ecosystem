"""Pure verification for the fresh, authority-free G77-256LZ Phase A."""
...
VERIFIER = ROOT / (
    ...
    "g77_256lz_fresh_wrong_scope_phase_a_review_object_v1/analysis/"
    "G77_256LZ_PHASE_A_REVIEW_OBJECT_VERIFIER_V1.py"
)
SPEC = importlib.util.spec_from_file_location("g77_256lz_verifier", VERIFIER)
...
def test_exactly_one_fresh_review_object_has_inner_and_whole_hashes() -> None:
    ...
    assert len(list(V.LZ.glob("*PHASE_A_REVIEW_OBJECT*.json"))) == 1
...
def test_exact_ly_context_scope_and_single_mismatch_are_bound() -> None:
...
def test_authority_human_decision_operation_and_effect_boundary() -> None:
    ...
    assert review["PROTECTED_EFFECT_STATE"] == "NONE_IN_LZ"
    assert set(review["STATE_COUNTERS"].values()) == {0}
    assert review["CANDIDATE_HUMAN_ACT_MATERIAL"]["is_authority"] is False


def test_lq_lv_decision_and_lr_lw_authority_nonreuse_and_unknown_preserved() -> None:
    ...
    assert previous["LV_APPROVAL_REUSED"] is False
    assert previous["LW_AUTHORITY_REUSED"] is False
    assert {
        previous["LW_DENIAL_CLASS"],
        previous["LW_P11_ENTRY_COUNT"],
        previous["LW_PROTECTED_INVOCATION_COUNT"],
        previous["LW_PROTECTED_EFFECT_COUNT"],
    } == {"UNKNOWN"}
...
def test_lt_lu_ly_are_readiness_not_authority_or_operational_proof() -> None:
    ...
    dependency = V.load_canonical(V.REVIEW)["review_object"][
        "LY_PREAUTHORITY_READINESS_DEPENDENCY"
    ]
    assert dependency == V.ly_readiness_dependency()
    assert dependency["required_for_future_execution"] is True
    assert dependency["future_controller_integration_state"].startswith("SEPARATE_FUTURE")
    assert dependency["static_proof_is_operational_proof"] is False


def test_human_authorizes_only_scope_a_and_scope_b_is_test_presentation() -> None:
    candidate = V.load_canonical(V.REVIEW)["review_object"][
        "CANDIDATE_HUMAN_ACT_MATERIAL"
    ]
    assert candidate["proposed_authority_scope"] == V.AUTHORIZED_SCOPE
    assert candidate["intentionally_different_future_runtime_presented_scope"] == V.PRESENTED_SCOPE
    assert candidate["human_authorizes_presented_scope"] is False
...
def test_noncanonical_and_duplicate_json_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(V.LZVerificationError, match="NONCANONICAL_JSON"):
        V.load_canonical(pretty)
    duplicate = tmp_path / "duplicate.json"
    duplicate.write_text('{"schema_id":"a","schema_id":"b"}\n')
    with pytest.raises(V.LZVerificationError, match="DUPLICATE_JSON_KEY"):
...
def test_terminal_decision_contains_required_boundary_and_frontier() -> None:
    ...
    assert decision["MINIMUM_LEGAL_NEXT_DELTA"] == "INDEPENDENT_HUMAN_AUTHENTICATION_OF_LZ"
