...
SCHEMA_ID = "G77_256DW_RAW_EXECUTION_EVIDENCE_V1"
GENERATION_ID = "G77_256DW_ONE_FRESH_BOUNDED_NON_PRODUCTION_E05_CONCURRENCY_P11_GENERATION_V1"
ATTEMPT_ID = "G77_256DW_E05_CONCURRENCY_ATTEMPT_001"
ACT_ID = "G77_256DW_EXACT_CURRENT_ONE_USE_HUMAN_OPERATIONAL_ACT_001"
CASE_ID = "G77_256DW_E05_TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS_EXACTLY_ONE_WINNER_001"
CHECKOUT = Path("/mnt/aigol")
RAW_ROOT = Path("/mnt/g77-evidence")
RAW_PATH = RAW_ROOT / "G77_256DW_RAW_EXECUTION_EVIDENCE_V1.jsonl"
DW_HARNESS_PATH = Path("/mnt/dp-harness/G77_256DW_P11_OPERATIONAL_HARNESS_V1.py")
...
PRE_ACT_SEAL_PATH = RAW_ROOT / "G77_256DW_PRE_ACT_CHECKPOINT_V1.json"
AUTHORITY_SEAL_PATH = RAW_ROOT / "G77_256DW_AUTHORITY_CHECKPOINT_V1.json"
GUEST_SEAL_PATH = RAW_ROOT / "G77_256DW_GUEST_EXECUTION_SEAL_V1.json"
TEARDOWN_SEAL_PATH = RAW_ROOT / "G77_256DW_GUEST_TEARDOWN_SEAL_V1.json"
CONTINUATION_MANIFEST_PATH = RAW_ROOT / "G77_256DW_CONTINUATION_MANIFEST_V1.json"
FIXTURE_ROOT = Path("/run/g77-256dw-p11")
...
def update_continuation_manifest(
    *,
    ...
    elif authority_lifecycle_state == "AUTHORIZED_BUT_NOT_CREATED":
        authority_state = {
            "lifecycle_state": "AUTHORIZED_NOT_CREATED", "act_identity": None,
            "owner_revision": None, "authority_survives": False,
            "transferable": False, "reusable": False,
        }
    elif authority_lifecycle_state.startswith("AVAILABLE_REVISION_0"):
        authority_state = {
            "lifecycle_state": "AVAILABLE", "act_identity": ACT_ID,
            "owner_revision": 0, "authority_survives": True,
            "transferable": False, "reusable": False,
        }
    elif authority_lifecycle_state.startswith("CONSUMED_REVISION_2"):
        authority_state = {
            "lifecycle_state": "CONSUMED", "act_identity": ACT_ID,
            "owner_revision": 2, "authority_survives": False,
            "transferable": False, "reusable": False,
        }
    else:
        authority_state = {
    ...
    manifest.update({
        ...
        "frontier_state": {
            "constitutional_frontier": "ONE_AUTHORIZED_DW_E05_CONCURRENCY_GENERATION",
...
def create_input_and_authority(gate: Any, bindings: Any, store: Any) -> tuple[Any, ...]:
    ...
    contract_hash = replay_hash({"contract": "G77_256DW_E05_CONCURRENCY_FAIL_CLOSED_AUTHORITY_V1"})
    input_value = {
        ...
        "input_identity": "G77_256DW_E05_CONCURRENCY_INPUT_001",
        "provenance_identity": "G77_256DW_AUTHENTICATED_DP_CD_MINIMUM_PROVENANCE_V1",
        "contract_identity": "G77_256DW_E05_CONCURRENCY_FAIL_CLOSED_AUTHORITY_CONTRACT_V1",
        ...
        "p10_inventory_identity": "G77_256DW_P10_ONE_ATTEMPT_ZERO_RETRY_INVENTORY_V1",
        "comparator_outcome_identity": "G77_256DW_E05_CONCURRENCY_EQUAL_OUTCOME_001",
        "comparator_outcome": "EQUAL",
        "replay_context_identity": "G77_256DW_EXISTING_RUNTIMELEDGER_REPLAY_CONTEXT_V1",
    }
    ...
    payload = {
        ...
        "case_id": CASE_ID,
        "evidence_run_identity": "G77_256DW_EVIDENCE_RUN_001",
        ...
        "minimum_retention": "CD_AUTHORIZED_MINIMUM_TRAIL_PLUS_DW_RAW_PREIMAGE",
    }
    ...
    act = CanonicalHumanAuthorityActV1(
        ...
        interaction_identity="G77_256DW_INTERACTION_001",
        conversation_identity="G77_256DW_CONVERSATION_001",
        session_identity="G77_256DW_SESSION_001",
        actor_identity="HUMAN_CONSTITUTIONAL_AUTHORITY",
        request_identity="G77_256DW_REQUEST_001",
        continuation_identity="G77_256DW_CONTINUATION_001",
        ...
        metadata={
            ...
            "human_authorization_source": "G77_256DW_CURRENT_PROMPT",
    ...
    correlation = create_canonical_che_evidence_correlation_v1(
        ...
        source_channel_identity="G77_256DW_HUMAN_AUTHORIZATION_CHANNEL",
        adapter_identity="G77_256DW_CANONICAL_BINDING_ADAPTER",
        request_identity=act.request_identity,
        che_entry_identity="G77_256DW_CHE_ENTRY_001",
        source_act_identity=act.authority_act_identity,
        source_act_digest=replay_hash(act.to_dict()),
        order_identity="G77_256DW_ORDER_001",
        idempotency_identity="G77_256DW_IDEMPOTENCY_001",
        ...
        authority_result_identity="G77_256DW_AUTHORITY_RESULT_001",
        ...
        owner_projection_identity="G77_256DW_OWNER_PROJECTION_001",
        failure_identity=NOT_APPLICABLE,
        presentation_identity="G77_256DW_PRESENTATION_001",
        response_identity="G77_256DW_RESPONSE_001",
        response_digest=replay_hash({"response": "G77_256DW_AUTHORITY_RECORDED"}),
        delivery_record_identity="G77_256DW_DELIVERY_001",
...
def custody_process(
    ...
    try:
        ...
        if receive_message(reader)["command"] != "INVOKE_CONCURRENTLY":
            raise RuntimeError("authority checkpoint acknowledgement missing")
        caller_connections = [server.accept()[0] for _ in range(2)]
        peer_facts: list[dict[str, Any]] = []
        for index, connection in enumerate(caller_connections, start=1):
            ...
            peer_facts.append({
                "contender_identity": f"G77_256DW_E05_CONTENDER_{index:03d}",
                "peer_pid": peer_pid,
                "peer_uid": peer_uid,
                "peer_gid": peer_gid,
                "observed_starting_owner_state": "AVAILABLE",
                "observed_starting_owner_revision": available.revision,
            })

        # S6 concurrency injector: each worker passes peer/act/input validation,
        # emits PRECLAIM, and then blocks immediately inside the protected claim.
        # Releasing both barriers proves both contenders observed AVAILABLE/0;
        # the existing exclusive revision create remains the sole linearizer.
        barriers: list[tuple[tuple[int, int], tuple[int, int], tuple[int, int]]] = [
            (os.pipe(), os.pipe(), os.pipe()) for _ in range(2)
        ]
        worker_pids: list[int] = []
        for index, connection in enumerate(caller_connections):
            (ready_read, ready_write), (release_read, release_write), (
                result_read, result_write,
            ) = barriers[index]
            worker_pid = os.fork()
            if worker_pid == 0:
                for other_index, other_connection in enumerate(caller_connections):
                    if other_index != index:
                        other_connection.close()
                for pipe_index, pipe_set in enumerate(barriers):
                    for descriptor in (
                        pipe_set[0][0], pipe_set[0][1], pipe_set[1][0],
                        pipe_set[1][1], pipe_set[2][0], pipe_set[2][1],
                    ):
                        keep = {
                            ready_write, release_read, result_write,
                        }
                ...
                def synchronized_claim(
                    ...
                    *,
                    ...
                    if os.read(release_read, 1) != b"G":
                        raise RuntimeError("concurrency barrier release missing")
                ...
                store.claim = synchronized_claim  # type: ignore[method-assign]
                contender_identity = peer_facts[index]["contender_identity"]
                request = CustodyRequest(
                    protocol_identity="P11_DA_DISPOSABLE_LOCAL_IPC_V1",
                    operation=CustodyOperation.CLAIM_AND_INVOKE_ONCE,
                    request_identity=f"G77_256DW_CUSTODY_REQUEST_{index + 1:03d}",
                ...
                try:
                    ...
                    outcome = {
                        **peer_facts[index],
                        ...
                        "authentication_state": "PASS__KERNEL_PEER_ACT_CHE_INPUT_AND_BINDING",
                        "claim_result": "WINNER",
                        "operational_authority_obtained": True,
                    ...
                    outcome = {
                        **peer_facts[index],
                        "request_identity": request.request_identity,
                        "authentication_state": "PASS__REACHED_SYNCHRONIZED_CLAIM_AFTER_AUTHENTICATION",
                        "claim_result": "LOSER_FAIL_CLOSED",
                        "operational_authority_obtained": False,
                    ...
                    connection.close()
                    os.write(result_write, canonical_bytes(outcome))
        ...
        for connection in caller_connections:
            connection.close()
        readiness = [os.read(pipe_set[0][0], 1) for pipe_set in barriers]
        if readiness != [b"R", b"R"]:
            raise RuntimeError("both authenticated contenders did not reach claim barrier")
        for pipe_set in barriers:
            os.write(pipe_set[1][1], b"G")
            os.close(pipe_set[1][1])
            os.close(pipe_set[0][0])

        contender_results: list[dict[str, Any]] = []
        for worker_pid, pipe_set in zip(worker_pids, barriers):
            raw_result = b""
            while True:
                chunk = os.read(pipe_set[2][0], 65536)
                if not chunk:
                    break
                raw_result += chunk
            os.close(pipe_set[2][0])
            waited, worker_status = os.waitpid(worker_pid, 0)
            if waited != worker_pid or worker_status != 0:
                raise RuntimeError("concurrency contender worker failed")
            contender_results.append(json.loads(raw_result))
        ...
        ledger_entries = consumer._ledger.read(gate.materialization_identity)
        winners = [item for item in contender_results if item["claim_result"] == "WINNER"]
        losers = [
            item for item in contender_results
            if item["claim_result"] == "LOSER_FAIL_CLOSED"
        ]
        send_message(control, {
            "message_type": "CONCURRENCY_COMPLETE",
            "case_id": CASE_ID,
            "contender_results": contender_results,
            "winner_count": len(winners),
            "loser_count": len(losers),
            "concurrency_invariant_pass": len(winners) == 1 and len(losers) == 1,
            "owner_state": {
...
def main() -> int:
    if len(sys.argv) != 6:
        raise SystemExit("expected DW_HARNESS_SHA SCHEMA_SHA HEAD TREE DN_HARNESS_SHA")
    ...
    if any(path.exists() for path in expected_absent):
        raise SystemExit("DT evidence sink is not empty")
    if not CONTINUATION_MANIFEST_PATH.is_file():
        raise SystemExit("DT continuation manifest is absent")
    ...
    try:
        actual_harness = sha256_path(DW_HARNESS_PATH)
        if actual_harness != expected_harness:
            raise RuntimeError("DT harness hash mismatch")
        ...
        evidence_id = append_record("commissioning_P03", "EVIDENCE", {
            ...
            "dn_instrument_scope": "CURRENT_DW_ENVIRONMENT_PROSPECTIVE_ONLY",
        ...
        evidence_id = append_record("commissioning_P11", "HUMAN_DECISION", {
            "generation_authorization": "PRESENT__EXACT_DW_ONLY",
        ...
        preact = {
            "schema_id": "G77_256DW_PRE_ACT_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "SAME_LIVE_DW_GENERATION_ONLY__NO_REPLAY__NO_AUTHORITY_TRANSFER",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DW_PRE_ACT_CHECKPOINT_V1",
                "sha256": preact_sha,
            },),
        )
        ...
        counters["human_operational_act_submitted_count"] = 1
        append_record("human_operational_act_available", "EVIDENCE", submitted)
        authority_seal = {
            "schema_id": "G77_256DW_AUTHORITY_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            "attempt_identity": ATTEMPT_ID,
            "authority_act_identity": ACT_ID,
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "EXACT_CURRENT_ACT_AVAILABLE_ONLY_INSIDE_SAME_LIVE_DW_GENERATION__NO_TRANSFER_OR_REPLAY",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            teardown_state="PENDING",
            authorized_next_action="SAME_LIVE_GENERATION_ONLY__RELEASE_EXACT_TWO_AUTHENTICATED_CLAIM_CONTENDERS__FRESH_SESSION_MUST_NOT_RESUME_ACT",
            additional_completed_seals=({
                "identity": "G77_256DW_AUTHORITY_CHECKPOINT_V1",
        ...
        send_message(parent_control, {"command": "INVOKE_CONCURRENTLY"})
        caller_clients = [connect_as_role("caller", ENDPOINT) for _ in range(2)]
        for _, caller_ready in caller_clients:
            os.read(caller_ready, 1)
            os.close(caller_ready)
        result = receive_message(reader)
        for caller_pid, _ in caller_clients:
            os.waitpid(caller_pid, 0)
        if result["message_type"] != "CONCURRENCY_COMPLETE":
        ...
        counters.update({
            ...
            "e05_concurrency_winner_count": result["winner_count"],
            "e05_concurrency_loser_count": result["loser_count"],
        })
        if not result["concurrency_invariant_pass"]:
            raise RuntimeError("E05 concurrency invariant failed")
        ...
        guest_seal = {
            "schema_id": "G77_256DW_GUEST_EXECUTION_SEAL_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [
            ...
            "evidence_obligation_id": "P11-E05",
            "case_id": CASE_ID,
            "operational_result": "PASS__TWO_AUTHENTICATED_CONCURRENT_CONTENDERS__EXACTLY_ONE_WINNER__LOSER_FAILED_CLOSED__ONE_EQUAL_ZERO_ROUTING_OUTPUT__ACT_CONSUMED",
            "execution_counters": counters,
            "raw_evidence_prefix_sha256": sha256_path(RAW_PATH),
            "raw_record_count": SEQUENCE,
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            first_failure_or_current_result="PASS__E05_CONCURRENCY__EXACTLY_ONE_WINNER__ONE_LOSER__ACT_CONSUMED",
            teardown_state="PENDING",
            authorized_next_action="TEARDOWN_AND_FINALIZATION_ONLY__NO_EXECUTION_REPLAY",
            additional_completed_seals=({
                "identity": "G77_256DW_GUEST_EXECUTION_SEAL_V1",
        ...
        teardown_seal = {
            "schema_id": "G77_256DW_GUEST_TEARDOWN_SEAL_V1",
            "generation_identity": GENERATION_ID,
        ...
        if first_failure is None and counters["e05_case_execution_count"] == 1:
            ...
            current_result = "PASS__E05_CONCURRENCY__EXACTLY_ONE_WINNER__GUEST_TEARDOWN_COMPLETE"
        ...
        update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DW_GUEST_TEARDOWN_SEAL_V1",
