#!/usr/bin/env python3
"""Materialize the nonauthority G77-256IY FUTURE Human barrier.

The only executable path authenticates committed IX, reuses the certified IF
...
IY_ROOT = ROOT / ".github/governance/evidence/g77_256iy_future_operational_v1"
LIVE = IY_ROOT / "live_binding"
OPERATION_ROOT = IY_ROOT / "operation_state"
TRANSIENT_ROOT = Path("/tmp/g77_256iy_future_operational_v1")

BRANCH = "g77-256fl-wrong-attempt-preboot-blocker"
ORIGIN = "git@github.com:Aljosa3/sapianta-ecosystem.git"
HEAD = "0d36155469c7d2fe043678a82652132c2878ff0e"
TREE = "1a8b2f988704dfc5c11df81450ebe487355d1450"
SUBJECT = "G77-256IX certify FUTURE post-commit import-root readiness"
...
IX_REPORT = IX_ROOT / "G77_256IX_G48_IMPLEMENTATION_REPORT_V1.md"
IX_TERMINAL = IX_ROOT / "G77_256IX_SPCE_TERMINAL_REPOSITORY_ONLY_REDUCTION_V1.json"
IX_FORMALIZER = IX_ROOT / "analysis/G77_256IX_POST_COMMIT_FUTURE_IMPORT_ROOT_READINESS_FORMALIZER_V1.py"
IX_TEST = IX_ROOT / "tests/test_g77_256ix_post_commit_future_import_root_readiness_v1.py"
IX_HASHES = {
    IX_REPORT: "1c2d78ba36aa23bf7c81f43ae830ea940500e6d570f2d5c2d34ffabcd143f13c",
    IX_TERMINAL: "4265d5455cf7febbf7ca7be2ea54802cf56cc27ce10856c13686f175bc244321",
    IX_FORMALIZER: "ab2502c2ab9ee8ddf043bc6257e3aa81c7796b341e13b6bb4c50e9aa0dad485e",
    IX_TEST: "c54199fdb6f4ef5514472faaa34aaea16a24179b3879c220ec6894b6d8fb5512",
}
...
class IYBarrierError(ValueError):
    """One deterministic fail-closed IY barrier error."""
...
def load_module(relative: Path, name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise IYBarrierError(f"MODULE_UNAVAILABLE:{relative}")
...
FM = load_module(FM_PATH, "g77_256iy_fm")
GN = load_module(GN_PATH, "g77_256iy_gn")
GL = load_module(GL_PATH, "g77_256iy_gl")
DU = load_module(DU_PATH, "g77_256iy_du_v2")
EB = load_module(EB_PATH, "g77_256iy_eb_v2")
EE = load_module(EE_PATH, "g77_256iy_ee_v2")
...
def unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, item in pairs:
        if key in value:
            raise IYBarrierError(f"DUPLICATE_KEY:{key}")
        value[key] = item
    return value
...
def load_canonical(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or raw != canonical_bytes(value):
        raise IYBarrierError(f"NONCANONICAL_JSON:{path}")
    return value
...
def write_json(path: Path, value: dict[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        raise IYBarrierError(f"FRESH_ARTIFACT_COLLISION:{path}")
...
def authenticate_entry(remote_head: str, nested_remote_tag: str) -> dict[str, Any]:
    ...
    if any(observed[key] != expected for key, expected in expected.items()):
        raise IYBarrierError("EXACT_COMMITTED_IX_ENTRY_MISMATCH")
    ...
    for label, revision in LINEAGE.items():
        if subprocess.run(
            ...
            raise IYBarrierError(f"LINEAGE_MISSING:{label}")
    ...
    if nested_state != {
        ...
        raise IYBarrierError("NESTED_AUTHORITY_MISMATCH")
    return observed | {"local_remote_equality": "VERIFIED", "lineage": lineage, "nested_authority": nested_state}


def reconstruct_ix() -> dict[str, Any]:
    identities: dict[str, Any] = {}
    for relative, expected_sha in IX_HASHES.items():
        committed = subprocess.check_output(["git", "show", f"{HEAD}:{relative}"], cwd=ROOT)
        current = (ROOT / relative).read_bytes()
        if committed != current or sha256_bytes(committed) != expected_sha:
            raise IYBarrierError(f"IX_COMMITTED_BYTE_MISMATCH:{relative}")
        identities[relative.name] = {
            "git_blob": git("rev-parse", f"{HEAD}:{relative}"),
            "sha256": expected_sha,
        }
    envelope = load_canonical(ROOT / IX_TERMINAL)
    if envelope["reduction_sha256"] != sha256_bytes(canonical_bytes(envelope["reduction"])):
        raise IYBarrierError("IX_INNER_SEAL_MISMATCH")
    reduction = envelope["reduction"]
    if reduction["terminal"] != "A__FUTURE_POST_COMMIT_IMPORT_ROOT_FULL_STATIC_READINESS_VERIFIED":
        raise IYBarrierError("IX_TERMINAL_MISMATCH")
    ...
    for key in (
        ...
        "full_static_preoperational_readiness",
        "post_commit_import_root_readiness",
    ):
        if readiness[key] != "VERIFIED":
            raise IYBarrierError(f"IX_READINESS_RECONSTRUCTION_MISMATCH:{key}")
    continuation = reduction["iv_iw_continuation"]
    import_root = continuation["import_root"]
    owner_graph = continuation["owner_graph"]
    if (
        owner_graph["selector_binding"]["cloud_init_sha256"]
        != "10092e4d10327c0bef42608e3125ca4b04b82b8e91a0c5e88a5148ee1a14fee2"
        or owner_graph["selector_binding"]["seed_sha256"]
        != "655b8b4122f89acbf0e4d3a670ee3b4fb38fb37600eeb6c8745c0a13cdc57eab"
        or import_root["with_governed_guest_import_root"] != "PASS"
        or reduction["route_firewall"]["parallel_flow_created"] != "VERIFIED__NO"
        ...
        or reduction["e05"]["after"] != "10/18"
    ):
        raise IYBarrierError("IX_TERMINAL_BOUNDARY_MISMATCH")
    return {
...
def reconstruct_ir() -> dict[str, Any]:
    ...
    for relative, expected_sha in IR_HASHES.items():
        ...
        current = (ROOT / relative).read_bytes()
        if committed != current or sha256_bytes(committed) != expected_sha:
            raise IYBarrierError(f"IR_COMMITTED_BYTE_MISMATCH:{relative}")
        identities[relative.name] = {
            "git_blob": git("rev-parse", f"{HEAD}:{relative}"),
            "sha256": expected_sha,
        }
...
def authenticate_future_semantics() -> dict[str, Any]:
    ...
    if envelope["binding_sha256"] != sha256_bytes(canonical_bytes(binding)):
        raise IYBarrierError("IF_ACT_CHE_SEAL_MISMATCH")
...
def derive_identity() -> dict[str, Any]:
    ...
    if IY_ROOT.name != expected_root:
        raise IYBarrierError("EVIDENCE_NAMESPACE_IDENTITY_MISMATCH")
    if FM.fresh_context.operation_vector(GENERATION) != VECTOR:
        raise IYBarrierError("CLOSED_VECTOR_IDENTITY_DERIVATION_FAILED")
    for identity in (GENERATION, OPERATION):
        ...
        if result.returncode not in (0, 1) or result.stdout:
            raise IYBarrierError(f"COMMITTED_IDENTITY_COLLISION:{identity}")
    return {
...
def materialize_v2_binding() -> dict[str, Any]:
    ...
    if sha256_path(source) != FUTURE_CANDIDATE_SHA:
        raise IYBarrierError("IF_CANDIDATE_IDENTITY_MISMATCH")
    ...
    readiness_candidate = readiness_root / "candidate/G77_256IY_V2_READINESS_CANDIDATE_V2.json"
    ...
    harness = bindings / "G77_256IY_EE_PATH_PROJECTION_FIXTURE_V1.py"
    eb_path = bindings / "G77_256IY_EB_RECEIPT_V2.json"
    ee_path = bindings / "G77_256IY_EE_RECEIPT_V2.json"
    for parent in (
    ...
    if set(du_result.values()) != {"PASS"} or eb_result["overall_result"] != "PASS":
        raise IYBarrierError("DU_EB_V2_BINDING_FAILED")
    if ee_result["pre_materialization_runtime_path_binding_result"] != "PASS":
        raise IYBarrierError("EE_V2_BINDING_FAILED")
    ...
    if (target["head"], target["tree"]) != (IF_HEAD, IF_TREE) or baseline != {"head": HEAD, "tree": TREE}:
        raise IYBarrierError("V2_RUNTIME_CERTIFICATION_ROLE_BINDING_FAILED")
    if ee["receipt"]["runtime_target_selection_binding"] != target or ee["receipt"]["certification_baseline"] != baseline:
        raise IYBarrierError("EB_EE_ROLE_DISAGREEMENT")
    return {
...
def materialize(args: argparse.Namespace) -> None:
    if LIVE.exists() or OPERATION_ROOT.exists() or TRANSIENT_ROOT.exists():
        raise IYBarrierError("IV_ONE_SHOT_NAMESPACE_NOT_FRESH")
    ...
    static = sealed(
        "G77_256IY_PREAUTHORITY_STATIC_READINESS_ENVELOPE_V1", "proof",
        {
            "schema_id": "G77_256IY_PREAUTHORITY_STATIC_READINESS_V1",
            "recorded_at_utc": recorded, "entry": entry,
            "ix_reconstruction": ix, "ir_reconstruction": ir,
    ...
    static_path = IY_ROOT / "G77_256IY_PREAUTHORITY_STATIC_READINESS_V1.json"
    write_json(static_path, static)

    observation = GL.prepare_and_observe_receipt_parent(ROOT, context)
    observation_path = IY_ROOT / "G77_256IY_GL_RECEIPT_PARENT_OBSERVATION_V1.json"
    ...
    equivalence = sealed(
        "G77_256IY_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256IY_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION, **gl_result},
    )
    equivalence_path = IY_ROOT / "G77_256IY_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1.json"
    write_json(equivalence_path, equivalence)

    counters = zero_counters()
    checkpoint_inner = {
        "schema_id": "G77_256IY_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1",
        ...
        "ix_terminal_reconstruction": ix, "ir_terminal_reconstruction": ir,
    ...
    checkpoint = sealed(
        "G77_256IY_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_ENVELOPE_V1",
        "checkpoint", checkpoint_inner,
    )
    checkpoint_path = IY_ROOT / "G77_256IY_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1.json"
    write_json(checkpoint_path, checkpoint)

    request_inner = {
        "schema_id": "G77_256IY_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1",
    ...
    request = sealed("G77_256IY_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_ENVELOPE_V1", "request", request_inner)
    request_path = IY_ROOT / "G77_256IY_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1.json"
    write_json(request_path, request)
    presentation = GN.render_human_authorization_presentation(request_path)
    presentation_path = IY_ROOT / "G77_256IY_HUMAN_OPERATIONAL_AUTHORIZATION_PRESENTATION_V1.txt"
    if presentation_path.exists() or presentation_path.is_symlink():
        raise IYBarrierError("FRESH_PRESENTATION_COLLISION")
    ...
    gn_proof = sealed(
        "G77_256IY_GN_HUMAN_PRESENTATION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256IY_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION,
    ...
    write_json(IY_ROOT / "G77_256IY_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1.json", gn_proof)

    counters["authorization_presentation"] = 1
    reduction = sealed(
        "G77_256IY_PREHUMAN_PHASE_A_REDUCTION_ENVELOPE_V1", "reduction",
        {
            "schema_id": "G77_256IY_PREHUMAN_PHASE_A_REDUCTION_V1",
            "recorded_at_utc": recorded, "generation_identity": GENERATION,
            "operation_identity": OPERATION,
            "terminal": "HUMAN_AUTHORIZATION_REQUIRED",
            "entry": entry, "ix_reconstruction": ix,
    ...
    write_json(IY_ROOT / "G77_256IY_PREHUMAN_PHASE_A_REDUCTION_V1.json", reduction)
