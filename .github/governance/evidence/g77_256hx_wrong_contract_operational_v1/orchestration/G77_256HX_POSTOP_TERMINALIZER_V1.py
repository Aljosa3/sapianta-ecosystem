#!/usr/bin/env python3
"""Finalization-only reduction and teardown for consumed G77-256HX."""
...
RUNTIME = HX / "operation_state/runtime_export"
RECEIPTS = HX / "operation_state/receipts"
TRANSIENT = Path("/tmp/g77_256hx_wrong_contract_operational_v1")
SERIAL_SOURCE = TRANSIENT / "serial.log"
SERIAL = HX / "G77_256HX_SERIAL_CONSOLE_V1.log"
PRE = RECEIPTS / "G77_256HX_PRE_EXECUTED_QEMU_ARGV_RECEIPT_V1.json"
POST = RECEIPTS / "G77_256HX_POST_EXECUTED_QEMU_ARGV_RECEIPT_V1.json"
RAW = RUNTIME / "G77_256HX_RAW_EXECUTION_EVIDENCE_V1.jsonl"
GUEST_EXECUTION = RUNTIME / "G77_256HX_GUEST_EXECUTION_SEAL_V1.json"
GUEST_TEARDOWN = RUNTIME / "G77_256HX_GUEST_TEARDOWN_SEAL_V1.json"
TERMINAL_MANIFEST = RUNTIME / "G77_256HX_CONTINUATION_MANIFEST_TERMINAL_V1.json"
AUTHORITY = HX / "G77_256HX_FRESH_HUMAN_OPERATIONAL_AUTHORIZATION_HANDOFF_V1.json"
AUTHORITY_CHECKPOINT = HX / "G77_256HX_AUTHORITY_VALIDATION_AND_CONSUMPTION_CHECKPOINT_V1.json"
...
GRANT = HX / "G77_256HX_HUMAN_OPERATIONAL_AUTHORIZATION_SOURCE_V1.txt"
CONTEXT = HX / "live_binding/SAPIANTA_FRESH_OPERATION_CONTEXT_V1.json"
NORMALIZATION = HX / "G77_256HX_HR_OPERATIONAL_EVIDENCE_NORMALIZATION_V1.json"
AUTHORITATIVE = HX / "G77_256HX_HR_AUTHORITATIVE_OPERATIONAL_REDUCTION_V1.json"
INDEPENDENT = HX / "G77_256HX_INDEPENDENT_OPERATIONAL_REDUCTION_V1.json"
AGREEMENT = HX / "G77_256HX_REDUCER_AGREEMENT_V1.json"
PRE_TEARDOWN = HX / "G77_256HX_SPCE_HOST_PRE_TEARDOWN_CHECKPOINT_V1.json"
TEARDOWN = HX / "G77_256HX_SPCE_HOST_TEARDOWN_CHECKPOINT_V1.json"
FINAL_SEAL = HX / "G77_256HX_SPCE_FINAL_EXECUTION_SEAL_V1.json"
TERMINAL = HX / "G77_256HX_SPCE_TERMINAL_REDUCTION_V1.json"
...
HR_REDUCER_PATH = ROOT / ".github/governance/evidence/g77_256hr_wrong_contract_formalization_v1/reducer/G77_256HR_WRONG_CONTRACT_REPOSITORY_CAPABILITY_REDUCER_V1.py"
SUCCESS = (
    "VERIFIED__G77_256HX_ONE_FRESH_HUMAN_AUTHORIZED_WRONG_CONTRACT_OPERATIONAL_"
    "COMMISSIONING__ONE_AUTHORITY__ONE_PRE__ONE_FM__ONE_NO_NETWORK_QEMU__"
    "ONE_VM_BOOT__WRONG_CONTRACT_REQUEST_DENIED_AT_D2_BEFORE_P11_ENTRY_"
    "INVOCATION_OR_EFFECT__ZERO_RETRY__AUTHORITATIVE_AND_INDEPENDENT_REDUCERS_"
    "AGREE__E05_9_OF_18__HUMAN_REVIEW_REQUIRED"
)
...
def load_module(path: Path):
    specification = importlib.util.spec_from_file_location("g77_256hx_hr_reducer", path)
    if specification is None or specification.loader is None:
        raise RuntimeError("HR reducer load failed")
...
def main() -> int:
    ...
    denial = one(records, "wrong_contract_denial_complete")
    ...
    if not (
        len(records) == 31
        and guest["operational_result"] == "PASS__WRONG_CONTRACT_DENIED_AT_D2_BEFORE_PRECLAIM_AND_ENTRY_WITH_ZERO_EFFECT"
        ...
        and terminal_manifest["first_failure_or_current_result"] == "PASS__E05_WRONG_CONTRACT_DENIAL__GUEST_TEARDOWN_COMPLETE"
        ...
        and differing == ["contract_identity", "record_identity"]
        ...
        and facts["wrong_contract_invariant_pass"] is True
        ...
        and attempt_facts["result"] == "PASS__ONE_VALID_ACT__ONE_ISOLATED_WRONG_CONTRACT_REQUEST_DENIED_AT_D2_BEFORE_PRECLAIM_ENTRY_CLAIM_INVOCATION_OR_EFFECT"
    ...
    normalized_packet = {
        "schema_id": "G77_256HX_WRONG_CONTRACT_OPERATIONAL_EVIDENCE_V1",
    ...
    normalization = {
        "schema_id": "G77_256HX_HR_OPERATIONAL_EVIDENCE_NORMALIZATION_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        "purpose": "LOSSLESS_ACTUAL_VALUE_MAPPING_FROM_HX_RAW_RECORDS_TO_HR_OWNED_WRONG_CONTRACT_SEMANTICS",
    ...
    write_new(NORMALIZATION, seal("G77_256HX_HR_OPERATIONAL_EVIDENCE_NORMALIZATION_ENVELOPE_V1", "normalization", normalization))
    ...
    authoritative_result = {
        ...
        "result": "PASS__COMPLETE_WRONG_CONTRACT_D2_DENIAL_EVIDENCE",
        "formal_spec_status": "VERIFIED",
        "hr_semantic_owner_status": "VERIFIED",
    ...
    authoritative = {
        "schema_id": "G77_256HX_HR_AUTHORITATIVE_OPERATIONAL_REDUCTION_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        "authoritative_reducer_status": "VERIFIED",
        "reducer_path": HR_REDUCER_PATH.relative_to(ROOT).as_posix(),
        "reducer_sha256": sha256(HR_REDUCER_PATH),
        "input_normalization_file_sha256": sha256(NORMALIZATION),
        "result": authoritative_result,
        "operational_criterion_satisfied": True,
        "recorded_at_utc": now(),
    }
    write_new(AUTHORITATIVE, seal("G77_256HX_HR_AUTHORITATIVE_OPERATIONAL_REDUCTION_ENVELOPE_V1", "reduction", authoritative))

    independent_records = [json.loads(line) for line in RAW.read_text(encoding="utf-8").splitlines()]
    independent_denial = one(independent_records, "wrong_contract_denial_complete")["facts"]
    ...
    independent = {
        "schema_id": "G77_256HX_INDEPENDENT_OPERATIONAL_REDUCTION_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        "independent_reducer_status": "VERIFIED",
        "independent_reducer_result": "ACCEPT",
        "independent_reducer_verdict": "PASS__ONE_WRONG_CONTRACT_REQUEST_DENIED_AT_D2_BEFORE_P11_ENTRY_INVOCATION_OR_EFFECT",
        "independent_source": "DIRECT_HX_RAW_RECORD_RECEIPT_SERIAL_AND_GUEST_SEAL_RECONSTRUCTION",
    ...
    write_new(INDEPENDENT, seal("G77_256HX_INDEPENDENT_OPERATIONAL_REDUCTION_ENVELOPE_V1", "reduction", independent))

    agreement = {
        "schema_id": "G77_256HX_REDUCER_AGREEMENT_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        ...
        "agreement": "VERIFIED__BOTH_ACCEPT_OPERATIONAL_WRONG_CONTRACT_CRITERION__E05_CREDIT_1",
        "one_shot_contract_satisfied": True,
        "e05": {"before": "8/18", "credit": 1, "after": "9/18"},
        "authoritative_reduction_file_sha256": sha256(AUTHORITATIVE),
        "independent_reduction_file_sha256": sha256(INDEPENDENT),
        "recorded_at_utc": now(),
    }
    write_new(AGREEMENT, seal("G77_256HX_REDUCER_AGREEMENT_ENVELOPE_V1", "agreement", agreement))
    ...
    copy_serial()
    pre_teardown = {
        "schema_id": "G77_256HX_SPCE_HOST_PRE_TEARDOWN_CHECKPOINT_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        "durable_evidence": {
    ...
    write_new(PRE_TEARDOWN, seal("G77_256HX_SPCE_HOST_PRE_TEARDOWN_CHECKPOINT_ENVELOPE_V1", "checkpoint", pre_teardown))

    if TRANSIENT.resolve() != Path("/tmp/g77_256hx_wrong_contract_operational_v1"):
    ...
    teardown = {
        "schema_id": "G77_256HX_SPCE_HOST_TEARDOWN_CHECKPOINT_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
    ...
    write_new(TEARDOWN, seal("G77_256HX_SPCE_HOST_TEARDOWN_CHECKPOINT_ENVELOPE_V1", "checkpoint", teardown))
    ...
    final = {
        "schema_id": "G77_256HX_SPCE_FINAL_EXECUTION_SEAL_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
    ...
    write_new(FINAL_SEAL, seal("G77_256HX_SPCE_FINAL_EXECUTION_SEAL_ENVELOPE_V1", "seal", final))
    terminal = {
        "schema_id": "G77_256HX_SPCE_TERMINAL_REDUCTION_V1",
        "generation_identity": GENERATION,
        "operation_identity": OPERATION,
        ...
        "reuse_impact": {
            "reused_certified_capability_set": "HW_HV_HT_HR_FM_GN_GL_HG_DU_EB_EE_P11_CHE_FK_EX",
            "new_capability_set": "HX_OPERATIONAL_EVIDENCE_ONLY",
    ...
    write_new(TERMINAL, seal("G77_256HX_SPCE_TERMINAL_REDUCTION_ENVELOPE_V1", "reduction", terminal))
    print(json.dumps({
        "result": SUCCESS,
        "serial_sha256": sha256(SERIAL),
