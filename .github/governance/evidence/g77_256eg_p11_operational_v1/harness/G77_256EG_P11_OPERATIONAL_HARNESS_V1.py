#!/usr/bin/env python3
"""One fresh bounded EG WRONG_CALLER attempt with durable raw evidence."""
...
SCHEMA_ID = "G77_256EG_RAW_EXECUTION_EVIDENCE_V1"
GENERATION_ID = "G77_256EG_ONE_FRESH_BOUNDED_NON_PRODUCTION_E05_WRONG_CALLER_P11_GENERATION_V1"
ATTEMPT_ID = "G77_256EG_E05_WRONG_CALLER_ATTEMPT_001"
AUTHORITY_REFERENCE = "G77_256EG_WRONG_CALLER_NON_AUTHORITY_REFERENCE_001"
CASE_ID = "G77_256EG_E05_WRONG_CALLER_DENIAL_BEFORE_ATTEMPT_001"
CHECKOUT = Path("/mnt/aigol")
RAW_ROOT = Path("/mnt/g77-evidence")
RAW_PATH = RAW_ROOT / "G77_256EG_RAW_EXECUTION_EVIDENCE_V1.jsonl"
EG_HARNESS_PATH = Path("/mnt/dp-harness/G77_256EG_P11_OPERATIONAL_HARNESS_V1.py")
...
PRE_ACT_SEAL_PATH = RAW_ROOT / "G77_256EG_PRE_ACT_CHECKPOINT_V1.json"
GUEST_SEAL_PATH = RAW_ROOT / "G77_256EG_GUEST_EXECUTION_SEAL_V1.json"
TEARDOWN_SEAL_PATH = RAW_ROOT / "G77_256EG_GUEST_TEARDOWN_SEAL_V1.json"
CONTINUATION_MANIFEST_PATH = RAW_ROOT / "G77_256EG_CONTINUATION_MANIFEST_V1.json"
TERMINAL_CONTINUATION_MANIFEST_PATH = (
    RAW_ROOT / "G77_256EG_CONTINUATION_MANIFEST_TERMINAL_V1.json"
)
FIXTURE_ROOT = Path("/run/g77-256eg-p11")
...
def update_continuation_manifest(
    *,
    ...
    manifest.update({
        ...
        "frontier_state": {
            "constitutional_frontier": "ONE_AUTHORIZED_EG_E05_WRONG_CALLER_GENERATION",
...
def create_wrong_caller_input(gate: Any, bindings: Any) -> tuple[bytes, dict[str, Any]]:
    """Create one canonical input presented by the wrong live fixed principal."""
    ...
    contract_hash = replay_hash(
        {"contract": "G77_256EG_E05_WRONG_CALLER_FAIL_CLOSED_V1"}
    )
    input_value = {
        ...
        "input_identity": "G77_256EG_E05_WRONG_CALLER_INPUT_001",
        "provenance_identity": "G77_256EG_AUTHENTICATED_CD_DY_MINIMUM_PROVENANCE_V1",
        "contract_identity": "G77_256EG_E05_WRONG_CALLER_FAIL_CLOSED_CONTRACT_V1",
        ...
        "p10_inventory_identity": "G77_256EG_P10_ONE_DENIAL_ZERO_RETRY_INVENTORY_V1",
        "comparator_outcome_identity": "G77_256EG_E05_WRONG_CALLER_DENIAL_OUTCOME_001",
        "comparator_outcome": "FAILED_CLOSED",
        "replay_context_identity": "G77_256EG_EXISTING_RUNTIMELEDGER_REPLAY_CONTEXT_V1",
    }
    ...
    try:
        ...
        if command == "INVOKE_WRONG_CALLER":
            ...
            request = CustodyRequest(
                ...
                request_identity="G77_256EG_WRONG_CALLER_CUSTODY_REQUEST_001",
            ...
            send_message(control, {
                "message_type": "WRONG_CALLER_COMPLETE",
                "case_id": CASE_ID,
                "selected_e05_vector": "WRONG_CALLER",
                ...
                "denial_point": "D1_FIXED_PEER_CREDENTIAL_AUTHENTICATION_BEFORE_D2_RESOLUTION_AND_PRECLAIM",
        ...
        raise RuntimeError("wrong-caller execution command missing")
    except BaseException as exc:
        try:
            send_message(control, {
...
def main() -> int:
    if len(sys.argv) != 6:
        raise SystemExit("expected EG_HARNESS_SHA SCHEMA_SHA HEAD TREE DN_HARNESS_SHA")
    ...
    expected_absent = (
        ...
        GUEST_SEAL_PATH, TEARDOWN_SEAL_PATH, TERMINAL_CONTINUATION_MANIFEST_PATH,
    )
    if any(path.exists() for path in expected_absent):
        raise SystemExit("EG evidence sink is not empty")
    if not CONTINUATION_MANIFEST_PATH.is_file():
        raise SystemExit("EG continuation manifest is absent")
    ...
    try:
        actual_harness = sha256_path(EG_HARNESS_PATH)
        if actual_harness != expected_harness:
            raise RuntimeError("EG harness hash mismatch")
        ...
        evidence_id = append_record("commissioning_P03", "EVIDENCE", {
            ...
            "dn_instrument_scope": "CURRENT_EG_ENVIRONMENT_PROSPECTIVE_ONLY",
        ...
        evidence_id = append_record("commissioning_P11", "HUMAN_DECISION", {
            "generation_authorization": "PRESENT__EXACT_EG_WRONG_CALLER_CASE_ONLY",
            "one_use_attempt_authorization": "NOT_REQUIRED__D1_DENIAL_PRECEDES_AUTHORITY_RESOLUTION",
        ...
        preact = {
            "schema_id": "G77_256EG_PRE_ACT_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [f"P{number:02d}" for number in range(1, 13)],
            "pending_gates": ["ONE_WRONG_CALLER_D1_DENIAL", "TEARDOWN"],
            "authority_disposition": "NO_ACT_AUTHORIZED_OR_CREATED__WRONG_PRINCIPAL_IS_NOT_AUTHORITY",
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "SAME_LIVE_EG_GENERATION_ONLY__NO_REPLAY__NO_AUTHORITY_TRANSFER",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            teardown_state="PENDING",
            authorized_next_action="SAME_LIVE_GENERATION_ONLY__PRESENT_ONE_WRONG_FIXED_PRINCIPAL_FOR_D1_FAIL_CLOSED_DENIAL",
            additional_completed_seals=({
                "identity": "G77_256EG_PRE_ACT_CHECKPOINT_V1",
                "sha256": preact_sha,
            },),
        )
        append_record("continuation_manifest_pre_act", "EVIDENCE", {
            "path": str(TERMINAL_CONTINUATION_MANIFEST_PATH),
        ...
        send_message(parent_control, {"command": "INVOKE_WRONG_CALLER"})
        caller_pid, caller_ready = connect_as_role("issuance", ENDPOINT)
        os.read(caller_ready, 1)
        os.close(caller_ready)
        result = receive_message(reader)
        os.waitpid(caller_pid, 0)
        if result["message_type"] != "WRONG_CALLER_COMPLETE":
            raise RuntimeError(f"custody failed wrong-caller case: {result}")
        ...
        if not result["denial_invariant_pass"]:
            raise RuntimeError("E05 wrong-caller denial invariant failed")
        append_record("p11_attempt_result", "FACT", {
            **result,
            ...
            "execution_counters": counters,
            "result": "PASS__WRONG_CALLER_REJECTED_AT_D1_PEER_AUTHENTICATION_BEFORE_D2_AND_PRECLAIM__NO_ACT_CLAIM_INVOCATION_LEDGER_OR_ROUTING_EFFECT",
        })
        guest_seal = {
            "schema_id": "G77_256EG_GUEST_EXECUTION_SEAL_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [
                "P01-P12", "WRONG_CALLER_D1_AUTHENTICATION_DENIAL",
                "ZERO_UNAUTHORIZED_EFFECT",
            ],
            "pending_gates": ["TEARDOWN", "G48_FINALIZATION"],
            "authority_disposition": "NO_HUMAN_ACT_CREATED__WRONG_PRINCIPAL_NEVER_REACHED_AUTHORITY_RESOLUTION",
            "evidence_obligation_id": "P11-E05",
            "case_id": CASE_ID,
            "selected_e05_vector": "WRONG_CALLER",
            "operational_result": "PASS__WRONG_CALLER_DENIED_AT_D1_BEFORE_D2_AND_PRECLAIM_WITH_ZERO_EFFECT",
            "execution_counters": counters,
            "raw_evidence_prefix_sha256": sha256_path(RAW_PATH),
            "raw_record_count": SEQUENCE,
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            current_spce_phase="PHASE_C_WRONG_CALLER_DENIAL_COMPLETE_PENDING_GUEST_TEARDOWN",
            execution_counters=counters,
            authority_lifecycle_state="NOT_CREATED",
            first_failure_or_current_result="PASS__E05_WRONG_CALLER__DENIED_AT_D1_BEFORE_D2_AND_PRECLAIM__ZERO_EFFECT",
            teardown_state="PENDING",
            authorized_next_action="TEARDOWN_AND_FINALIZATION_ONLY__NO_EXECUTION_REPLAY_OR_NEXT_VECTOR",
            additional_completed_seals=({
                "identity": "G77_256EG_GUEST_EXECUTION_SEAL_V1",
        ...
        teardown_seal = {
            "schema_id": "G77_256EG_GUEST_TEARDOWN_SEAL_V1",
            "generation_identity": GENERATION_ID,
        ...
        if first_failure is None and counters["e05_case_execution_count"] == 1:
            ...
            current_result = "PASS__E05_WRONG_CALLER__DENIED_AT_D1_BEFORE_D2_AND_PRECLAIM__GUEST_TEARDOWN_COMPLETE"
        ...
        update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256EG_GUEST_TEARDOWN_SEAL_V1",
