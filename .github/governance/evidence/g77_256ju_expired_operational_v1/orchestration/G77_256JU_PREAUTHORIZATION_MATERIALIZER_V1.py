#!/usr/bin/env python3
"""Materialize the nonauthority G77-256JU EXPIRED Human barrier.

This path authenticates committed JT, materializes one fresh JU-local
candidate/context and operation state, derives the sealed GN presentation,
and stops.  It contains no authority-consumption or operational-launch call.
"""
...
LIVE = JU / "live_binding"
OPERATION_ROOT = JU / "operation_state"
TRANSIENT_ROOT = Path("/tmp/g77_256ju_expired_operational_v1")

BRANCH = "g77-256fl-wrong-attempt-preboot-blocker"
ORIGIN = "git@github.com:Aljosa3/sapianta-ecosystem.git"
HEAD = "deae45e9ff85ebcde4153ade567c8dbf0ac6b729"
TREE = "4708ba5da7ea269ba89e920a049007671b9851d2"
SUBJECT = "G77-256JT repair EXPIRED bootstrap checkout binding"
...
class JUBarrierError(RuntimeError):
    """One deterministic fail-closed JU preauthorization error."""


def load_module(relative: Path, name: str) -> ModuleType:
    specification = importlib.util.spec_from_file_location(name, ROOT / relative)
    if specification is None or specification.loader is None:
        raise JUBarrierError(f"MODULE_UNAVAILABLE:{relative}")
...
FM = load_module(FM_PATH, "g77_256ju_fm")
GL = load_module(GL_PATH, "g77_256ju_gl")
GN = load_module(GN_PATH, "g77_256ju_gn")
...
def write_json(path: Path, value: dict[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        raise JUBarrierError(f"FRESH_ARTIFACT_COLLISION:{path}")
...
def authenticate_entry(remote_head: str, nested_remote_tag: str) -> dict[str, Any]:
    ...
    if any(observed[key] != value for key, value in expected.items()):
        raise JUBarrierError("EXACT_RATIFIED_JT_ENTRY_MISMATCH")
    for line in git("status", "--porcelain", "--untracked-files=all").splitlines():
        if not line.startswith("?? " + JU.relative_to(ROOT).as_posix() + "/"):
            raise JUBarrierError("JU_BOUNDED_WORKTREE_SCOPE_VIOLATION")
    if subprocess.run(
        ...
        raise JUBarrierError("CONSTITUTIONAL_ANCESTRY_MISMATCH")
    ...
    if nested_state != {
        ...
        raise JUBarrierError("NESTED_AUTHORITY_MISMATCH")
    return observed | {
...
def authenticate_jt() -> dict[str, Any]:
    for path, expected_hash in EXPECTED_HASHES.items():
        ...
        if current != committed or sha256_bytes(current) != expected_hash:
            raise JUBarrierError(f"COMMITTED_JT_DEPENDENCY_MISMATCH:{path}")
    ...
    if not isinstance(reduction, dict) or envelope.get("reduction_sha256") != (
        ...
        raise JUBarrierError("JT_TERMINAL_REDUCTION_SEAL_MISMATCH")
    ...
    if (
        ...
        raise JUBarrierError("JT_CONSTITUTIONAL_BOUNDARY_MISMATCH")
    if FM.governed_checkout_identity(ROOT, VECTOR, HEAD, TREE) != (JR_HEAD, JR_TREE):
        raise JUBarrierError("JT_STABLE_EXPIRED_CHECKOUT_MISMATCH")
    ...
    if command[2:4] != (JR_HEAD, JR_TREE):
        raise JUBarrierError("JT_BOOTSTRAP_COMMAND_MISMATCH")
    for member, source in (
        ...
        if projected != (ROOT / source).read_bytes():
            raise JUBarrierError(f"JT_NOCLOUD_PROJECTION_MISMATCH:{member}")
    return {
...
def authenticate_expired_semantics() -> dict[str, Any]:
    adapter = load_module(JR_ADAPTER, "g77_256ju_expired_adapter")
    ...
    if coordinates != {
        ...
        raise JUBarrierError("EXPIRED_COORDINATE_MISMATCH")
    ...
    for token in (
        ...
        if token not in transformed:
            raise JUBarrierError("EXPIRED_SPECIALIZATION_MISMATCH")
    return {
        "vector": VECTOR,
...
def materialize(args: argparse.Namespace) -> None:
    if LIVE.exists() or OPERATION_ROOT.exists() or TRANSIENT_ROOT.exists():
        raise JUBarrierError("JU_ONE_SHOT_NAMESPACE_NOT_FRESH")
    ...
    if sha256_path(source_candidate) != FM.CANDIDATE_SHA256:
        raise JUBarrierError("FM_CANONICAL_CANDIDATE_MISMATCH")
    ...
    if candidate.read_bytes() != runtime.read_bytes():
        raise JUBarrierError("CANDIDATE_RUNTIME_BYTE_MISMATCH")
    ...
    if (
        ...
        raise JUBarrierError("JU_CONTEXT_ROLE_OR_TEMPORAL_BINDING_MISMATCH")
    ...
    try:
        ...
        raise JUBarrierError("ADVANCING_JT_CHECKOUT_SUBSTITUTION_ACCEPTED")
    ...
    static = sealed(
        "G77_256JU_PREAUTHORITY_STATIC_READINESS_ENVELOPE_V1",
        "proof",
        {
            "schema_id": "G77_256JU_PREAUTHORITY_STATIC_READINESS_V1",
            ...
            "fresh_candidate": {
                ...
                "fresh_ju_candidate_instance": True,
    ...
    static_path = JU / "G77_256JU_PREAUTHORITY_STATIC_READINESS_V1.json"
    write_json(static_path, static)

    observation = GL.prepare_and_observe_receipt_parent(ROOT, context)
    observation_path = JU / "G77_256JU_GL_RECEIPT_PARENT_OBSERVATION_V1.json"
    ...
    equivalence = sealed(
        "G77_256JU_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_ENVELOPE_V1",
        "proof",
        {
            "schema_id": "G77_256JU_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1",
            "generation_identity": GENERATION,
            "operation_identity": OPERATION,
            **gl_result,
        },
    )
    equivalence_path = JU / "G77_256JU_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1.json"
    ...
    checkpoint_inner = {
        "schema_id": "G77_256JU_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1",
    ...
    checkpoint = sealed(
        "G77_256JU_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_ENVELOPE_V1",
        "checkpoint",
        checkpoint_inner,
    )
    checkpoint_path = JU / "G77_256JU_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1.json"
    write_json(checkpoint_path, checkpoint)

    request_inner = {
        "schema_id": "G77_256JU_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1",
        "recorded_at_utc": recorded,
    ...
    request = sealed(
        "G77_256JU_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_ENVELOPE_V1",
        "request",
        request_inner,
    )
    request_path = JU / "G77_256JU_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1.json"
    write_json(request_path, request)
    presentation = GN.render_human_authorization_presentation(request_path)
    presentation_path = JU / "G77_256JU_HUMAN_OPERATIONAL_AUTHORIZATION_PRESENTATION_V1.txt"
    if presentation_path.exists() or presentation_path.is_symlink():
        raise JUBarrierError("FRESH_PRESENTATION_COLLISION")
    ...
    gn_proof = sealed(
        "G77_256JU_GN_HUMAN_PRESENTATION_EQUIVALENCE_ENVELOPE_V1",
        "proof",
        {
            "schema_id": "G77_256JU_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1",
            "generation_identity": GENERATION,
            "operation_identity": OPERATION,
    ...
    write_json(JU / "G77_256JU_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1.json", gn_proof)

    reduction = sealed(
        "G77_256JU_PREHUMAN_PHASE_A_REDUCTION_ENVELOPE_V1",
        "reduction",
        {
            "schema_id": "G77_256JU_PREHUMAN_PHASE_A_REDUCTION_V1",
            "recorded_at_utc": recorded,
            "generation_identity": GENERATION,
            "operation_identity": OPERATION,
            ...
            "governance_dashboard": {
                "project_progress": "VERIFIED__JU_EXACT_PREAUTHORIZATION_BARRIER_MATERIALIZED",
                ...
                "constitutional_continuation_progress": "VERIFIED__JT_TO_JU_PREAUTHORIZATION_STOP",
            },
            "ccwim": {
    ...
    write_json(JU / "G77_256JU_PREHUMAN_PHASE_A_REDUCTION_V1.json", reduction)
