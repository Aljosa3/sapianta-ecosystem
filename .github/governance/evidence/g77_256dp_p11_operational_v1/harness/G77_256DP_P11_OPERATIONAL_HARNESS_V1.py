...
SCHEMA_ID = "G77_256DP_RAW_EXECUTION_EVIDENCE_V1"
GENERATION_ID = "G77_256DP_ONE_FRESH_BOUNDED_NON_PRODUCTION_P11_GENERATION_V1"
ATTEMPT_ID = "G77_256DP_G1_E12_ATTEMPT_001"
ACT_ID = "G77_256DP_EXACT_CURRENT_ONE_USE_HUMAN_OPERATIONAL_ACT_001"
CHECKOUT = Path("/mnt/aigol")
RAW_ROOT = Path("/mnt/g77-evidence")
RAW_PATH = RAW_ROOT / "G77_256DP_RAW_EXECUTION_EVIDENCE_V1.jsonl"
DP_HARNESS_PATH = Path("/mnt/dp-harness/G77_256DP_P11_OPERATIONAL_HARNESS_V1.py")
...
PRE_ACT_SEAL_PATH = RAW_ROOT / "G77_256DP_PRE_ACT_CHECKPOINT_V1.json"
AUTHORITY_SEAL_PATH = RAW_ROOT / "G77_256DP_AUTHORITY_CHECKPOINT_V1.json"
GUEST_SEAL_PATH = RAW_ROOT / "G77_256DP_GUEST_EXECUTION_SEAL_V1.json"
TEARDOWN_SEAL_PATH = RAW_ROOT / "G77_256DP_GUEST_TEARDOWN_SEAL_V1.json"
FIXTURE_ROOT = Path("/run/g77-256dp-p11")
...
def create_input_and_authority(gate: Any, bindings: Any, store: Any) -> tuple[Any, ...]:
    ...
    contract_hash = replay_hash({"contract": "G77_256DP_G1_E12_COORDINATE_BINDING_V1"})
    input_value = {
        ...
        "input_identity": "G77_256DP_G1_E12_INPUT_001",
        "provenance_identity": "G77_256DP_AUTHENTICATED_DO_DK_DI_CH_CD_PROVENANCE_V1",
        "contract_identity": "G77_256DP_G1_E12_COORDINATE_BINDING_CONTRACT_V1",
        ...
        "p10_inventory_identity": "G77_256DP_P10_ONE_ATTEMPT_ZERO_RETRY_INVENTORY_V1",
        "comparator_outcome_identity": "G77_256DP_G1_E12_MISMATCH_OUTCOME_001",
        "comparator_outcome": "MISMATCH",
        "replay_context_identity": "G77_256DP_EXISTING_RUNTIMELEDGER_REPLAY_CONTEXT_V1",
    }
    ...
    payload = {
        ...
        "evidence_obligation_id": "P11-E12",
        "case_id": "G1_E12_COORDINATE_BINDING_ISOLATED_001",
        "evidence_run_identity": "G77_256DP_EVIDENCE_RUN_001",
        ...
        "minimum_retention": "CD_AUTHORIZED_MINIMUM_TRAIL_PLUS_DP_RAW_PREIMAGE",
    }
    ...
    act = CanonicalHumanAuthorityActV1(
        ...
        interaction_identity="G77_256DP_INTERACTION_001",
        conversation_identity="G77_256DP_CONVERSATION_001",
        session_identity="G77_256DP_SESSION_001",
        actor_identity="HUMAN_CONSTITUTIONAL_AUTHORITY",
        request_identity="G77_256DP_REQUEST_001",
        continuation_identity="G77_256DP_CONTINUATION_001",
        ...
        metadata={
            ...
            "human_authorization_source": "G77_256DP_CURRENT_PROMPT",
    ...
    correlation = create_canonical_che_evidence_correlation_v1(
        ...
        source_channel_identity="G77_256DP_HUMAN_AUTHORIZATION_CHANNEL",
        adapter_identity="G77_256DP_CANONICAL_BINDING_ADAPTER",
        request_identity=act.request_identity,
        che_entry_identity="G77_256DP_CHE_ENTRY_001",
        source_act_identity=act.authority_act_identity,
        source_act_digest=replay_hash(act.to_dict()),
        order_identity="G77_256DP_ORDER_001",
        idempotency_identity="G77_256DP_IDEMPOTENCY_001",
        ...
        authority_result_identity="G77_256DP_AUTHORITY_RESULT_001",
        ...
        owner_projection_identity="G77_256DP_OWNER_PROJECTION_001",
        failure_identity=NOT_APPLICABLE,
        presentation_identity="G77_256DP_PRESENTATION_001",
        response_identity="G77_256DP_RESPONSE_001",
        response_digest=replay_hash({"response": "G77_256DP_AUTHORITY_RECORDED"}),
        delivery_record_identity="G77_256DP_DELIVERY_001",
...
def custody_process(
    ...
    try:
        ...
        request = CustodyRequest(
            ...
            request_identity="G77_256DP_CUSTODY_REQUEST_001",
...
def main() -> int:
    if len(sys.argv) != 6:
        raise SystemExit("expected DP_HARNESS_SHA SCHEMA_SHA HEAD TREE DN_HARNESS_SHA")
    ...
    if any(path.exists() for path in expected_absent):
        raise SystemExit("DP evidence sink is not empty")
    ...
    try:
        actual_harness = sha256_path(DP_HARNESS_PATH)
        if actual_harness != expected_harness:
            raise RuntimeError("DP harness hash mismatch")
        ...
        evidence_id = append_record("commissioning_P03", "EVIDENCE", {
            ...
            "dn_instrument_scope": "CURRENT_DP_ENVIRONMENT_PROSPECTIVE_ONLY",
        ...
        evidence_id = append_record("commissioning_P11", "HUMAN_DECISION", {
            "generation_authorization": "PRESENT__EXACT_DP_ONLY",
        ...
        preact = {
            "schema_id": "G77_256DP_PRE_ACT_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            ...
            "continuation": "SAME_LIVE_DP_GENERATION_ONLY__NO_REPLAY__NO_AUTHORITY_TRANSFER",
        }
        ...
        authority_seal = {
            "schema_id": "G77_256DP_AUTHORITY_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            "attempt_identity": ATTEMPT_ID,
            "authority_act_identity": ACT_ID,
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "EXACT_CURRENT_ACT_AVAILABLE_ONLY_INSIDE_SAME_LIVE_DP_GENERATION__NO_TRANSFER_OR_REPLAY",
        }
        ...
        append_record("p11_attempt_result", "FACT", {
            **result,
            ...
            "case_id": "G1_E12_COORDINATE_BINDING_ISOLATED_001",
            "execution_counters": counters,
            "result": "PASS__ONE_ACCEPTED_ATTEMPT__MISMATCH_NON_ROUTING_OUTPUT__ACT_CONSUMED",
        })
        guest_seal = {
            "schema_id": "G77_256DP_GUEST_EXECUTION_SEAL_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [
        ...
        teardown_seal = {
            "schema_id": "G77_256DP_GUEST_TEARDOWN_SEAL_V1",
            "generation_identity": GENERATION_ID,
        ...
        write_canonical(TEARDOWN_SEAL_PATH, teardown_seal)
