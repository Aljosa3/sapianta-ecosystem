#!/usr/bin/env python3
"""One Human-authorized MA authority and one LT-supervised existing FM child."""
...
MA_REL = Path(".github/governance/evidence/g77_256ma_operational_wrong_scope_acceptance_v1")
MA = ROOT / MA_REL
LZ_REL = Path(".github/governance/evidence/g77_256lz_fresh_wrong_scope_phase_a_review_object_v1")
LZ = ROOT / LZ_REL
...
SOURCE = MA / "G77_256MA_HUMAN_OPERATIONAL_AUTHORIZATION_SOURCE_V1.txt"
TRANSITION = MA / "G77_256MA_LP_COMMITTED_REVIEW_TRANSITION_V1.json"
PREAUTH = MA / "G77_256MA_PREAUTHORITY_READINESS_CHECKPOINT_V1.json"
HANDOFF = MA / "G77_256MA_FRESH_HUMAN_OPERATIONAL_AUTHORITY_V1.json"
FM_BINDING = MA / "G77_256MA_FM_PRECONSUMPTION_INVOCATION_BINDING_V1.json"
LT_BINDING = MA / "G77_256MA_LT_SEALED_SUPERVISOR_INPUT_V1.json"
PRECONSUMPTION = MA / "G77_256MA_FINAL_PRECONSUMPTION_CHECKPOINT_V1.json"
CONSUMPTION = MA / "G77_256MA_AUTHORITY_CONSUMPTION_CHECKPOINT_V1.json"
LAUNCH_HANDOFF = MA / "G77_256MA_LT_LAUNCH_HANDOFF_V1.json"
LT_STATE = MA / "operation_state/lt_supervision/G77_256MA_WRONG_SCOPE_OPERATION_001"
OPERATION_ROOT = MA / "operation_state"
...
C_HEAD = "5f19fb7b549341e59fa679398473577b7441d011"
C_TREE = "2f278d36287e3822ec2bdb378dda33f6276704e4"
C_SUBJECT = "G77-256LZ record fresh Human decision readiness"
R_HEAD = "9940fd070226f0f0d2221d06a37e37a09bd28506"
R_TREE = "ad68043714dd31f01770e4718d74c3396a6a78dc"
R_SUBJECT = "G77-256LZ seal fresh WRONG_SCOPE Phase-A review object"
LU_HEAD = "3f6055f85c8bff0a3de89eda27421c92e0742904"
LY_HEAD = "0158178c45d66a81f675ba431fb8ae261c4b3bd9"
LY_TREE = "0ac5500055df6ba75dce76a2107604dae1be8608"
...
OBJECT_ID = "G77_256LZ_WRONG_SCOPE_PHASE_A_REVIEW_OBJECT_001"
LIFECYCLE_ID = "G77_256LZ_WRONG_SCOPE_PHASE_A_LIFECYCLE_001"
OBJECT_WHOLE_SHA256 = "e3f402fb9d8ccaa0495af55a4314e1656501f47e2814bf371d0146a87d78d3ed"
OBJECT_INNER_SHA256 = "3d70185525d4e35543f3f7336dd961bc2e8812291cf90cfd15e97f32a690fe5f"
...
READINESS_SHA256 = "6010e2c548da79d035fb962cfbbf56b26ab73cf5ad0fb024c71813512c2a85bb"
TRANSITION_SHA256 = "675ee4dcae532c704739f8b4cb78a6f7c8bf59f79dfecb8de9a24d449f92b6ac"
...
LY_SHA256 = "920e84f5be5fcb17f29dc7850c3f0862609ea847bde7fd72483c2d4e18f5e49d"
LY_DECISION_SHA256 = "fbf54d57a506adcf81ea830007300a87ad87d5bf78f987864e985ecce4567c47"
AUTHORIZED_SCOPE = "P11_DA_ONE_BOUNDED_OPERATIONAL_ATTEMPT_V1"
PRESENTED_SCOPE = "P11_DA_DIFFERENT_OPERATIONAL_SCOPE_V1"
GENERATION = "G77_256LZ_ONE_FRESH_HUMAN_AUTHORIZED_WRONG_SCOPE_OPERATIONAL_COMMISSIONING_V1"
OPERATION = "G77_256LZ_E05_WRONG_SCOPE_DENIAL_BEFORE_P11_ENTRY_001"
AUTHORITY_ID = "G77_256MA_FRESH_HUMAN_OPERATIONAL_AUTHORITY_001"
LT_LIFECYCLE_ID = "G77_256MA_WRONG_SCOPE_OPERATIONAL_LIFECYCLE_001"
LT_INVOCATION_ID = "G77_256MA_EXACT_FM_INVOCATION_001"
...
FM = load_module(FM_PATH, "g77_256ma_existing_fm_owner", FM_SHA256)
LG = load_module(LG_PATH, "g77_256ma_existing_lg_semantics", LG_SHA256)
LT = load_module(LT_PATH, "g77_256ma_existing_lt_supervisor", LT_SHA256)
LY = load_module(LY_PATH, "g77_256ma_existing_ly_readiness", LY_SHA256)
...
def allowed_worktree() -> None:
    allowed_lz = {Path("operation_state/receipts/G77_256LZ_PRE_EXECUTED_QEMU_ARGV_RECEIPT_V1.json"), Path("operation_state/receipts/G77_256LZ_POST_EXECUTED_QEMU_ARGV_RECEIPT_V1.json")}
    allowed_lz |= {Path("operation_state/runtime_export") / name for name in (
        "G77_256LZ_RAW_EXECUTION_EVIDENCE_V1.jsonl", "G77_256DN_P03_RAW_EVIDENCE_V1.jsonl", "G77_256DN_SPCE_EXECUTION_SEAL_V1.json", "G77_256LZ_PRE_ACT_CHECKPOINT_V1.json", "G77_256LZ_AUTHORITY_CHECKPOINT_V1.json", "G77_256LZ_GUEST_EXECUTION_SEAL_V1.json", "G77_256LZ_GUEST_TEARDOWN_SEAL_V1.json", "G77_256LZ_CONTINUATION_MANIFEST_TERMINAL_V1.json")}
    for line in git("status", "--porcelain=v1", "--untracked-files=all").splitlines():
        if line[:2] != "??":
            raise RuntimeError(f"TRACKED_OR_STAGED_MUTATION:{line}")
        path = line[3:]
        if path.startswith(MA_REL.as_posix() + "/"):
            continue
        if path.startswith(LZ_REL.as_posix() + "/") and Path(path[len(LZ_REL.as_posix()) + 1:]) in allowed_lz:
            continue
        raise RuntimeError(f"UNRELATED_UNTRACKED_MUTATION:{line}")


def authenticate_entry(remote_head: str, nested_remote_tag: str) -> None:
    if (ROOT != Path("/home/pisarna/work/sapianta-fl") or git("branch", "--show-current") != BRANCH or git("rev-parse", "HEAD") != C_HEAD or git("rev-parse", "HEAD^{tree}") != C_TREE or git("show", "-s", "--format=%s", "HEAD") != C_SUBJECT or remote_head != C_HEAD or git("rev-parse", f"{R_HEAD}^{{tree}}") != R_TREE or git("show", "-s", "--format=%s", R_HEAD) != R_SUBJECT or not is_ancestor(LY_HEAD, R_HEAD) or not is_ancestor(R_HEAD, C_HEAD) or git("status", "--porcelain=v1", "--untracked-files=no")):
        raise RuntimeError("MA_ENTRY_AUTHENTICATION_CONFLICT")
    allowed_worktree()
...
def authenticate_readiness_chain() -> None:
    if (git("rev-parse", f"{LT_HEAD}^{{tree}}") != LT_TREE or git("rev-parse", f"{LU_HEAD}^{{tree}}") != LU_TREE or git("rev-parse", f"{LY_HEAD}^{{tree}}") != LY_TREE or not is_ancestor(LT_HEAD, LU_HEAD) or not is_ancestor(LU_HEAD, LY_HEAD) or any(sha256_path(p) != h for p, h in ((LT_PATH, LT_SHA256), (LT_DECISION, LT_DECISION_SHA256), (LU_RELATION, LU_RELATION_SHA256), (LU_DECISION, LU_DECISION_SHA256), (LY_PATH, LY_SHA256), (LY_DECISION, LY_DECISION_SHA256)))):
        raise RuntimeError("LT_LU_LY_IDENTITY_CONFLICT")
...
def authenticate_lz() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(p.is_symlink() or not p.is_file() or sha256_path(p) != h for p, h in expected):
        raise RuntimeError("LZ_OBJECT_CONTEXT_PRESENTATION_READINESS_CONFLICT")
    ...
    if (not isinstance(review, dict) or envelope.get("review_object_sha256") != OBJECT_INNER_SHA256 or digest(review) != OBJECT_INNER_SHA256 or review.get("LIFECYCLE_ID") != LIFECYCLE_ID or review.get("REVIEW_OBJECT_ID") != OBJECT_ID or review.get("VECTOR") != "WRONG_SCOPE" or review.get("PHASE") != "A" or review.get("AUTHORIZED_SCOPE") != AUTHORIZED_SCOPE or review.get("PRESENTED_SCOPE") != PRESENTED_SCOPE or review.get("ONE_SHOT_LIMIT") != 1 or review.get("RETRY_LIMIT") != 0 or review.get("AUTHORITY_CREATED") != "NO" or review.get("AUTHORITY_CONSUMED") != "NO" or review.get("OPERATION_STATE") != "NOT_STARTED" or any(review.get("STATE_COUNTERS", {}).values())):
        raise RuntimeError("LZ_REVIEW_OBJECT_BINDING_CONFLICT")
    ...
    if (mismatch.get("field") != "authority_scope" or mismatch.get("expected") != AUTHORIZED_SCOPE or mismatch.get("presented") != PRESENTED_SCOPE or mismatch.get("independent_semantic_mutation_count") != 1):
        raise RuntimeError("LZ_ISOLATED_MISMATCH_CONFLICT")
    context = FM.fresh_context.load_context(CONTEXT, repository_root=ROOT)
    if (context.get("context_sha256") != CONTEXT_SHA256 or context.get("generation_identity") != GENERATION or context.get("operation_identity") != OPERATION or context.get("repository_head") != LY_HEAD or FM.context_vector(context) != "WRONG_SCOPE"):
        raise RuntimeError("LZ_CANONICAL_CONTEXT_CONFLICT")
...
def preflight(args: argparse.Namespace) -> None:
    ...
    if any(path.exists() for path in FM.receipt_consumable_paths(ROOT, context)) or Path(context["serial_path"]).exists():
        raise RuntimeError("MA_OPERATION_NAMESPACE_NOT_FRESH")
    OPERATION_ROOT.mkdir(mode=0o700)
    readiness = LY.prepare_lt_parent_readiness(OPERATION_ROOT, LT_STATE)
    if readiness.get("observation", {}).get("lifecycle_leaf_absent") is not True:
        raise RuntimeError("LY_PREAUTHORITY_READINESS_NOT_PROVEN")
    ly_file_sha = persist(LY_READINESS, readiness)
    transition_file_sha = persist(TRANSITION, proof)
    analysis = {
        "failure_novelty_and_convergence": {
            "FAILURE_CLASS": "PROOF_GAP",
            "NOVELTY": "FRESH_HUMAN_AUTHORIZED_OPERATIONAL_PROOF_FRONTIER__NO_CAPABILITY_NOVELTY",
            "AFFECTED_INVARIANT": "VALID_HUMAN_AUTHORITY_FOR_SCOPE_A_MUST_NOT_AUTHORIZE_PRESENTED_SCOPE_B",
            "PREVIOUS_CLOSEST_EDGE": "LY_STATIC_PARENT_READINESS_AND_PRECONSUMPTION_IDENTITY_CONTINUITY",
            "SEMANTIC_DIFFERENCE": "MA_MUST_CROSS_THE_OPERATIONAL_DENIAL_EDGE_ONCE_WITH_FRESH_NONTRANSFERABLE_AUTHORITY",
            ...
            "CONVERGENCE_SIGNAL": "EXISTING_LY_LT_LU_FM_AND_LG_MECHANISMS_AUTHENTICATED_AS_SUFFICIENT",
            "REPETITION_PRESSURE": "HIGH__FRESHNESS_REQUIRED_AFTER_HISTORICAL_AUTHORITY_WAS_SPENT",
            "VERIFICATION_AMPLIFICATION_RISK": "ELEVATED__EXACT_REUSE_AND_ONE_ATTEMPT_BOUND_REQUIRED",
            "OVERENGINEERING_RISK": "ELEVATED_IF_ANY_NEW_OWNER_ROUTE_SUPERVISOR_OR_GENERIC_ABSTRACTION_IS_ADDED",
        },
        "cross_vector_reuse_assessment": {
            "GK": "PARTIAL_REUSE__AUTHORITY_AND_OPERATIONAL_EVIDENCE_PATTERN_ONLY",
            "GL": "SEMANTICALLY_EQUIVALENT__CALLER_OWNED_PARENT_PREAUTHORIZATION_PATTERN",
            "LG": "VECTOR_SPECIFIC__EXACT_WRONG_SCOPE_PAIR_AND_SINGLE_AUTHORITY_SCOPE_MISMATCH",
            "LP": "EXACT_REUSE_POSSIBLE__COMMITTED_REVIEW_TO_CURRENT_ADMISSION_RELATION",
            "LQ": "PARTIAL_REUSE__PHASE_A_STRUCTURE_ONLY__DECISION_NONTRANSFERABLE",
            "LR": "NOT_APPLICABLE__HISTORICAL_OPERATIONAL_TERMINAL_ONLY",
            "LS": "PARTIAL_REUSE__SESSION_LIFETIME_FAILURE_LOCALIZATION_ONLY",
            "LT": "EXACT_REUSE_POSSIBLE__ONE_SHOT_LIFECYCLE_RESERVATION_AND_SUPERVISION",
            "LU": "EXACT_REUSE_POSSIBLE__AUTHENTICATED_LT_TO_EXACT_FM_INTEGRATION",
            "LV": "PARTIAL_REUSE__PHASE_A_PRECEDENT_ONLY__APPROVAL_NONTRANSFERABLE",
            "LW": "PARTIAL_REUSE__CONTROLLER_COMPOSITION_ONLY__AUTHORITY_SPENT_NONREUSABLE",
            "LX": "SEMANTICALLY_EQUIVALENT__DUPLICATE_BLOCKER_CLASSIFICATION",
            "LY": "EXACT_REUSE_POSSIBLE__PARENT_READINESS_AND_READ_ONLY_PRECONSUMPTION_REOBSERVATION",
            "LZ": "VECTOR_SPECIFIC__EXACT_FRESH_PHASE_A_IDENTITY_AND_HUMAN_DECISION_SOURCE",
            "WRONG_INPUT": "SEMANTICALLY_EQUIVALENT__NEGATIVE_AUTHORITY_BINDING_STRUCTURE",
            "WRONG_CONTRACT": "SEMANTICALLY_EQUIVALENT__NEGATIVE_AUTHORITY_BINDING_STRUCTURE",
            "WRONG_PROVENANCE": "PARTIAL_REUSE__PREENTRY_FAIL_CLOSED_PATTERN",
            "WRONG_CALLER": "PARTIAL_REUSE__PREENTRY_FAIL_CLOSED_PATTERN",
            "FUTURE": "PARTIAL_REUSE__TEMPORAL_BINDING_STRUCTURE",
            "EXPIRED": "PARTIAL_REUSE__FRESHNESS_STRUCTURE",
            "WRONG_SCOPE": "VECTOR_SPECIFIC__EXACT_SCOPE_PAIR_AND_ISOLATED_MISMATCH",
        },
        "spce": {
            "S": "LZ_INDEPENDENTLY_HUMAN_AUTHENTICATED_AND_EXPLICITLY_HUMAN_APPROVED__E05_12_OF_18__WRONG_SCOPE_UNSAT__LY_STATIC_READINESS_BLOCKER_CLOSED__NO_REUSABLE_HISTORICAL_AUTHORITY",
            "P": "OPERATIONAL_WRONG_SCOPE_ACCEPTANCE_REMAINS_UNPROVEN__FRESH_LZ_DECISION_NOT_YET_MATERIALIZED_AS_ONE_AUTHORITY_AND_ONE_ATTEMPT",
            "C": "AUTHENTICATED_TRUE__EXISTING_AUTHORITY_LIFECYCLE_LY_LT_LU_FM_AND_WRONG_SCOPE_DENIAL_MECHANISMS_SUFFICIENT__NO_NEW_CAPABILITY",
            "E": "ONE_FRESH_AUTHORITY__ONE_CONSUMPTION__ONE_LT_RESERVATION__AT_MOST_ONE_FM_CHILD__ONE_WRONG_SCOPE_PRESENTATION__DURABLE_TERMINAL_REDUCTION",
        },
        "ex": {"EX_REUSED": "VERIFIED__17_OF_17", "EX_RECONSTRUCTED": "VERIFIED__0"},
    }
    checkpoint = {"schema_id": "G77_256MA_PREAUTHORITY_READINESS_CHECKPOINT_V1", "recorded_at_utc": now(), "generation": "G77-256MA", "repository": {"branch": BRANCH, "review_head": R_HEAD, "review_tree": R_TREE, "current_head": C_HEAD, "current_tree": C_TREE, "remote_head": args.remote_head}, "lz": {"lifecycle_id": LIFECYCLE_ID, "review_object_id": OBJECT_ID, "review_object_sha256": OBJECT_WHOLE_SHA256, "canonical_inner_sha256": OBJECT_INNER_SHA256, "context_sha256": CONTEXT_SHA256, "context_whole_sha256": CONTEXT_WHOLE_SHA256, "presentation_sha256": PRESENTATION_SHA256}, "human_decision": {"source": "EXPLICIT_CURRENT_HUMAN_DECISION_OVER_INDEPENDENTLY_AUTHENTICATED_LZ_OBJECT", "source_sha256": source_sha, "count": 1, "authorized_scope": AUTHORIZED_SCOPE, "presented_scope": PRESENTED_SCOPE, "human_authorizes_presented_scope": False, "authorized_operation_count": 1, "authorized_retry_count": 0}, **analysis, "failure_class": "PROOF_GAP", "novelty": "FRESH_HUMAN_AUTHORIZED_OPERATIONAL_WRONG_SCOPE_PROOF_USING_EXISTING_LT_LU_LY_READINESS", "new_capability_required": "NO", "new_proof_required": "YES__AUTHENTICATED_TERMINAL_OPERATIONAL_WRONG_SCOPE_DENIAL_BEFORE_P11", "isolated_mismatch": "authority_scope", "isolated_mismatch_count": 1, "lp_transition_sha256": TRANSITION_SHA256, "lp_transition_file_sha256": transition_file_sha, "lt_capability_id": CAPABILITY_ID, "lt_head": LT_HEAD, "lu_head": LU_HEAD, "lu_integration_readiness": "PROVEN", "ly_head": LY_HEAD, "ly_tree": LY_TREE, "ly_readiness_file_sha256": ly_file_sha, "ly_readiness_observation_sha256": readiness["observation_sha256"], "ly_parent_identity": readiness["observation"]["parent_identity"], "ly_leaf_binding_sha256": readiness["observation"]["leaf_binding_sha256"], "fm_static_readiness_sha256": static["readiness_sha256"], "receipt_namespace_unused": True, "lt_namespace_fresh": True, "lq_approval_reused": False, "lv_approval_reused": False, "lr_authority_reused": False, "lw_authority_reused": False, "authority_created_count": 0, "authority_consumed_count": 0, "operation_attempt_count": 0, "retry_count": 0, "ready_for_authority_creation": True}
    persist(PREAUTH, seal("G77_256MA_PREAUTHORITY_READINESS_CHECKPOINT_ENVELOPE_V1", "checkpoint", checkpoint))
    print("A__G77_256MA_PREAUTHORITY_READY__ZERO_AUTHORITY__ZERO_OPERATION")


def prepare_authority(args: argparse.Namespace) -> None:
    authenticate_entry(args.remote_head, args.nested_remote_tag)
    authenticate_readiness_chain()
    _, context = authenticate_lz()
    source_sha = authenticate_human_source()
    proof = derive_transition(context)
    ...
    readiness = load_canonical(LY_READINESS)
    if readiness.get("observation_sha256") != checkpoint.get("ly_readiness_observation_sha256"):
        raise RuntimeError("LY_PREAUTHORITY_READINESS_DRIFT")
    if any(path.exists() or path.is_symlink() for path in (HANDOFF, FM_BINDING, LT_BINDING, PRECONSUMPTION, LY_REOBSERVATION, CONSUMPTION, LAUNCH_HANDOFF, LT_STATE)):
        raise RuntimeError("MA_AUTHORITY_NAMESPACE_NOT_FRESH")
    ...
    final = {"schema_id": "G77_256MA_FINAL_PRECONSUMPTION_CHECKPOINT_V1", "recorded_at_utc": now(), "generation": "G77-256MA", "authority_id": AUTHORITY_ID, "authority_path": HANDOFF.relative_to(ROOT).as_posix(), "authority_file_sha256": authority_sha, "authority_inner_sha256": handoff["authorization_sha256"], "authority_source_sha256": source_sha, "authority_state": "GRANTED_UNCONSUMED", "authority_valid": True, "authority_unconsumed": True, "object_id": OBJECT_ID, "object_whole_sha256": OBJECT_WHOLE_SHA256, "authorized_scope": AUTHORIZED_SCOPE, "presented_scope": PRESENTED_SCOPE, "human_authorizes_presented_scope": False, "isolated_mismatch": "authority_scope", "context_sha256": CONTEXT_SHA256, "current_admission_identity": f"{C_HEAD}/{C_TREE}", "final_admission_validation": "PASS", "admission_result": admission["result"], "ly_readiness_file_sha256": sha256_path(LY_READINESS), "ly_readiness_observation_sha256": readiness["observation_sha256"], "ly_preconsumption_reobservation_state": "REQUIRED_IMMEDIATELY_BEFORE_CONSUMPTION", "fm_binding_file_sha256": fm_file_sha, "fm_invocation_binding_sha256": fm_binding_sha, "final_fm_argv_sha256": digest(argv), "lt_capability_id": CAPABILITY_ID, "lt_lifecycle_id": LT_LIFECYCLE_ID, "lt_invocation_id": LT_INVOCATION_ID, "lt_input_file_sha256": lt_file_sha, "lt_input_inner_sha256": lt_envelope["binding_sha256"], "attempt_limit": 1, "retry_limit": 0, "lt_namespace_fresh": True, "lt_reservation_count": 0, "lt_child_launch_count": 0, "fm_operation_attempt_count": 0, "qemu_start_count": 0, "vm_start_count": 0, "authority_created_count": 1, "authority_consumed_count": 0}
    persist(PRECONSUMPTION, seal("G77_256MA_FINAL_PRECONSUMPTION_CHECKPOINT_ENVELOPE_V1", "checkpoint", final))
    print(json.dumps({"terminal": "A__G77_256MA_ONE_AUTHORITY_CREATED__FINAL_PRECONSUMPTION_READY__ZERO_OPERATION", "authority_id": AUTHORITY_ID, "authority_sha256": authority_sha, "lt_input_sha256": lt_file_sha}, sort_keys=True))
...
def consume_and_launch(args: argparse.Namespace) -> None:
    ...
    _, context = authenticate_lz()
    source_sha = authenticate_human_source()
    if any(path.exists() or path.is_symlink() for path in (LY_REOBSERVATION, CONSUMPTION, LAUNCH_HANDOFF, LT_STATE)):
        raise RuntimeError("MA_AUTHORITY_CONSUMED_OR_LT_RESERVED__NO_RELAUNCH")
    proof = derive_transition(context)
    if FM.load_committed_review_transition(TRANSITION, sha256_path(TRANSITION)) != proof:
        raise RuntimeError("DURABLE_LP_TRANSITION_DRIFT")
    handoff, authority_sha = FM.load_authority(HANDOFF)
    ...
    readiness = load_canonical(LY_READINESS)
    reobservation = LY.reobserve_before_authority_consumption(
        OPERATION_ROOT, LT_STATE, readiness
    )
    if reobservation.get("result") != LY.PRECONSUMPTION_RESULT:
        raise RuntimeError("LY_PRECONSUMPTION_REOBSERVATION_FAILED")
    persist(
        LY_REOBSERVATION,
        seal(
            "G77_256MA_LY_PRECONSUMPTION_REOBSERVATION_ENVELOPE_V1",
            "reobservation",
            {
                "schema_id": "G77_256MA_LY_PRECONSUMPTION_REOBSERVATION_V1",
                "recorded_at_utc": now(),
                "generation": "G77-256MA",
                "authority_id": AUTHORITY_ID,
                "authority_state": "GRANTED_UNCONSUMED",
                "current_admission_identity": f"{C_HEAD}/{C_TREE}",
                "ly_readiness_file_sha256": sha256_path(LY_READINESS),
                **reobservation,
            },
        ),
    )
    consumed = {"schema_id": "G77_256MA_AUTHORITY_CONSUMPTION_CHECKPOINT_V1", "recorded_at_utc": now(), "generation": "G77-256MA", "authority_id": AUTHORITY_ID, "authority_file_sha256": authority_sha, "authority_source_sha256": source_sha, "authority_state_before": "GRANTED_UNCONSUMED", "authority_state_after": "CONSUMED__NONREUSABLE", "authority_created_count": 1, "authority_consumed_count": 1, "authority_reusable": False, "operation_attempt_count_at_consumption": 0, "retry_count": 0, "lt_reservation_count_at_consumption": 0, "fm_operation_attempt_count_at_consumption": 0}
    persist(CONSUMPTION, seal("G77_256MA_AUTHORITY_CONSUMPTION_CHECKPOINT_ENVELOPE_V1", "checkpoint", consumed))
    handoff_result = LT.launch_detached(LT_STATE, LT_BINDING)
    launch = {"schema_id": "G77_256MA_LT_LAUNCH_HANDOFF_V1", "recorded_at_utc": now(), "generation": "G77-256MA", "authority_id": AUTHORITY_ID, "authority_state": "CONSUMED__NONREUSABLE", "lt_lifecycle_id": LT_LIFECYCLE_ID, "lt_invocation_id": LT_INVOCATION_ID, "lt_input_file_sha256": sha256_path(LT_BINDING), "lt_reservation_count": 1, "lt_child_launch_count_maximum": 1, "fm_operation_attempt_count_maximum": 1, "retry_count": 0, "handoff": handoff_result}
    persist(LAUNCH_HANDOFF, seal("G77_256MA_LT_LAUNCH_HANDOFF_ENVELOPE_V1", "handoff", launch))
