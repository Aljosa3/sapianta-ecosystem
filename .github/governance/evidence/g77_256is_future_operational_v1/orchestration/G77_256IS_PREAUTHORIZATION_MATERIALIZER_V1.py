#!/usr/bin/env python3
"""Materialize the nonauthority G77-256IS FUTURE Human barrier.

The only executable path authenticates committed IR, reuses the certified IF
...
LIVE = IS_ROOT / "live_binding"
OPERATION_ROOT = IS_ROOT / "operation_state"
TRANSIENT_ROOT = Path("/tmp/g77_256is_future_operational_v1")

BRANCH = "g77-256fl-wrong-attempt-preboot-blocker"
ORIGIN = "git@github.com:Aljosa3/sapianta-ecosystem.git"
HEAD = "8f68fb2e94db3c8bac56fc96315b312e6217dea6"
TREE = "b43a481b505c83b12ae006eccf1f35aa0e191eff"
SUBJECT = "G77-256IR certify FUTURE preauthorization readiness"
...
class ISBarrierError(ValueError):
    """One deterministic fail-closed IS barrier error."""
...
def load_module(relative: Path, name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise ISBarrierError(f"MODULE_UNAVAILABLE:{relative}")
...
FM = load_module(FM_PATH, "g77_256is_fm")
GN = load_module(GN_PATH, "g77_256is_gn")
GL = load_module(GL_PATH, "g77_256is_gl")
DU = load_module(DU_PATH, "g77_256is_du_v2")
EB = load_module(EB_PATH, "g77_256is_eb_v2")
EE = load_module(EE_PATH, "g77_256is_ee_v2")
...
def unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, item in pairs:
        if key in value:
            raise ISBarrierError(f"DUPLICATE_KEY:{key}")
        value[key] = item
    return value
...
def load_canonical(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or raw != canonical_bytes(value):
        raise ISBarrierError(f"NONCANONICAL_JSON:{path}")
    return value
...
def write_json(path: Path, value: dict[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        raise ISBarrierError(f"FRESH_ARTIFACT_COLLISION:{path}")
...
def authenticate_entry(remote_head: str, nested_remote_tag: str) -> dict[str, Any]:
    ...
    if any(observed[key] != expected for key, expected in expected.items()):
        raise ISBarrierError("EXACT_COMMITTED_IR_ENTRY_MISMATCH")
    ...
    for label, revision in LINEAGE.items():
        if subprocess.run(
            ...
            raise ISBarrierError(f"LINEAGE_MISSING:{label}")
    ...
    if nested_state != {
        ...
        raise ISBarrierError("NESTED_AUTHORITY_MISMATCH")
...
def reconstruct_ir() -> dict[str, Any]:
    ...
    for relative, expected_sha in IR_HASHES.items():
        ...
        if committed != current or sha256_bytes(committed) != expected_sha:
            raise ISBarrierError(f"IR_COMMITTED_BYTE_MISMATCH:{relative}")
...
def authenticate_future_semantics() -> dict[str, Any]:
    ...
    if envelope["binding_sha256"] != sha256_bytes(canonical_bytes(binding)):
        raise ISBarrierError("IF_ACT_CHE_SEAL_MISMATCH")
...
def derive_identity() -> dict[str, Any]:
    ...
    if IS_ROOT.name != expected_root:
        raise ISBarrierError("EVIDENCE_NAMESPACE_IDENTITY_MISMATCH")
    if FM.fresh_context.operation_vector(GENERATION) != VECTOR:
        raise ISBarrierError("CLOSED_VECTOR_IDENTITY_DERIVATION_FAILED")
    for identity in (GENERATION, OPERATION):
        ...
        if result.returncode not in (0, 1) or result.stdout:
            raise ISBarrierError(f"COMMITTED_IDENTITY_COLLISION:{identity}")
    return {
...
def materialize_v2_binding() -> dict[str, Any]:
    ...
    if sha256_path(source) != FUTURE_CANDIDATE_SHA:
        raise ISBarrierError("IF_CANDIDATE_IDENTITY_MISMATCH")
    ...
    readiness_candidate = readiness_root / "candidate/G77_256IS_V2_READINESS_CANDIDATE_V2.json"
    ...
    harness = bindings / "G77_256IS_EE_PATH_PROJECTION_FIXTURE_V1.py"
    eb_path = bindings / "G77_256IS_EB_RECEIPT_V2.json"
    ee_path = bindings / "G77_256IS_EE_RECEIPT_V2.json"
    for parent in (
    ...
    if set(du_result.values()) != {"PASS"} or eb_result["overall_result"] != "PASS":
        raise ISBarrierError("DU_EB_V2_BINDING_FAILED")
    if ee_result["pre_materialization_runtime_path_binding_result"] != "PASS":
        raise ISBarrierError("EE_V2_BINDING_FAILED")
    ...
    if (target["head"], target["tree"]) != (IF_HEAD, IF_TREE) or baseline != {"head": HEAD, "tree": TREE}:
        raise ISBarrierError("V2_RUNTIME_CERTIFICATION_ROLE_BINDING_FAILED")
    if ee["receipt"]["runtime_target_selection_binding"] != target or ee["receipt"]["certification_baseline"] != baseline:
        raise ISBarrierError("EB_EE_ROLE_DISAGREEMENT")
    return {
...
def materialize(args: argparse.Namespace) -> None:
    if LIVE.exists() or OPERATION_ROOT.exists() or TRANSIENT_ROOT.exists():
        raise ISBarrierError("IS_ONE_SHOT_NAMESPACE_NOT_FRESH")
    ...
    static = sealed(
        "G77_256IS_PREAUTHORITY_STATIC_READINESS_ENVELOPE_V1", "proof",
        {
            "schema_id": "G77_256IS_PREAUTHORITY_STATIC_READINESS_V1",
            "recorded_at_utc": recorded, "entry": entry, "ir_reconstruction": ir,
    ...
    static_path = IS_ROOT / "G77_256IS_PREAUTHORITY_STATIC_READINESS_V1.json"
    write_json(static_path, static)

    observation = GL.prepare_and_observe_receipt_parent(ROOT, context)
    observation_path = IS_ROOT / "G77_256IS_GL_RECEIPT_PARENT_OBSERVATION_V1.json"
    ...
    equivalence = sealed(
        "G77_256IS_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256IS_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION, **gl_result},
    )
    equivalence_path = IS_ROOT / "G77_256IS_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1.json"
    write_json(equivalence_path, equivalence)

    counters = zero_counters()
    checkpoint_inner = {
        "schema_id": "G77_256IS_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1",
        ...
        "ir_terminal_reconstruction": ir, "fresh_identity": identity,
        "future_semantics": future,
        "identities": {
    ...
    checkpoint = sealed(
        "G77_256IS_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_ENVELOPE_V1",
        "checkpoint", checkpoint_inner,
    )
    checkpoint_path = IS_ROOT / "G77_256IS_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1.json"
    write_json(checkpoint_path, checkpoint)

    request_inner = {
        "schema_id": "G77_256IS_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1",
    ...
    request = sealed("G77_256IS_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_ENVELOPE_V1", "request", request_inner)
    request_path = IS_ROOT / "G77_256IS_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1.json"
    write_json(request_path, request)
    presentation = GN.render_human_authorization_presentation(request_path)
    presentation_path = IS_ROOT / "G77_256IS_HUMAN_OPERATIONAL_AUTHORIZATION_PRESENTATION_V1.txt"
    if presentation_path.exists() or presentation_path.is_symlink():
        raise ISBarrierError("FRESH_PRESENTATION_COLLISION")
    ...
    gn_proof = sealed(
        "G77_256IS_GN_HUMAN_PRESENTATION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256IS_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION,
    ...
    write_json(IS_ROOT / "G77_256IS_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1.json", gn_proof)

    counters["authorization_presentation"] = 1
    reduction = sealed(
        "G77_256IS_PREHUMAN_PHASE_A_REDUCTION_ENVELOPE_V1", "reduction",
        {
            "schema_id": "G77_256IS_PREHUMAN_PHASE_A_REDUCTION_V1",
            "recorded_at_utc": recorded, "generation_identity": GENERATION,
            "operation_identity": OPERATION,
            "terminal": "A__WAITING_FOR_EXPLICIT_HUMAN_OPERATIONAL_AUTHORIZATION",
            "entry": entry, "ir_reconstruction": ir, "future_semantics": future,
    ...
    write_json(IS_ROOT / "G77_256IS_PREHUMAN_PHASE_A_REDUCTION_V1.json", reduction)
