#!/usr/bin/env python3
"""One fresh bounded EN CONSUMED reuse-denial case with durable evidence."""
...
SCHEMA_ID = "G77_256EP_RAW_EXECUTION_EVIDENCE_V1"
GENERATION_ID = "G77_256EP_ONE_FRESH_BOUNDED_NON_PRODUCTION_E05_CONSUMED_REUSE_P11_GENERATION_V1"
ATTEMPT_ID = "G77_256EP_E05_CONSUMED_REUSE_ATTEMPT_001"
ACT_ID = "G77_256EP_EXACT_CURRENT_ONE_USE_HUMAN_OPERATIONAL_ACT_001"
CASE_ID = "G77_256EP_E05_CONSUMED_AUTHORITY_REUSE_DENIAL_001"
CHECKOUT = Path("/mnt/aigol")
RAW_ROOT = Path("/mnt/g77-evidence")
RAW_PATH = RAW_ROOT / "G77_256EP_RAW_EXECUTION_EVIDENCE_V1.jsonl"
EN_HARNESS_PATH = Path("/mnt/dp-harness/G77_256EP_P11_OPERATIONAL_HARNESS_V1.py")
...
PRE_ACT_SEAL_PATH = RAW_ROOT / "G77_256EP_PRE_ACT_CHECKPOINT_V1.json"
AUTHORITY_SEAL_PATH = RAW_ROOT / "G77_256EP_AUTHORITY_CHECKPOINT_V1.json"
GUEST_SEAL_PATH = RAW_ROOT / "G77_256EP_GUEST_EXECUTION_SEAL_V1.json"
TEARDOWN_SEAL_PATH = RAW_ROOT / "G77_256EP_GUEST_TEARDOWN_SEAL_V1.json"
CONTINUATION_MANIFEST_PATH = RAW_ROOT / "G77_256EP_CONTINUATION_MANIFEST_V1.json"
TERMINAL_CONTINUATION_MANIFEST_PATH = RAW_ROOT / "G77_256EP_CONTINUATION_MANIFEST_TERMINAL_V1.json"
FIXTURE_ROOT = Path("/run/g77-256en-p11")
...
def update_continuation_manifest(
    *,
    ...
    source_path = (
        TERMINAL_CONTINUATION_MANIFEST_PATH
        if TERMINAL_CONTINUATION_MANIFEST_PATH.is_file()
        else CONTINUATION_MANIFEST_PATH
    )
    ...
    authority_state = {
        "lifecycle_state": (
            "NOT_CREATED" if authority_lifecycle_state == "NOT_CREATED"
            else "NO_AUTHORITY_SURVIVES"
        ),
        "act_identity": None,
        "owner_revision": None,
    ...
    for item in additional_completed_seals:
        observation = f"GUEST_CHECKPOINT_RECORDED__{item['identity']}__SHA256_{item['sha256']}"
    ...
    manifest.update({
        ...
        "frontier_state": {
            "constitutional_frontier": "ONE_AUTHORIZED_EN_E05_CONSUMED_REUSE_GENERATION",
            "exact_next_legal_action": authorized_next_action,
            "continuation_mode": "FINALIZATION_ONLY" if authorized_next_action.startswith(("TEARDOWN", "HOST_AUTHENTICATION")) else "SAME_LIVE_GENERATION_ONLY",
    ...
    file_sha = write_canonical_atomic(TERMINAL_CONTINUATION_MANIFEST_PATH, updated)
...
def create_input_and_authority(gate: Any, bindings: Any, store: Any) -> tuple[Any, ...]:
    ...
    contract_hash = replay_hash({"contract": "G77_256EP_E05_CONSUMED_REUSE_FAIL_CLOSED_AUTHORITY_V1"})
    input_value = {
        ...
        "input_identity": "G77_256EP_E05_CONSUMED_REUSE_INPUT_001",
        "provenance_identity": "G77_256EP_AUTHENTICATED_EM_CD_MINIMUM_PROVENANCE_V1",
        "contract_identity": "G77_256EP_E05_CONSUMED_REUSE_FAIL_CLOSED_AUTHORITY_CONTRACT_V1",
        ...
        "p10_inventory_identity": "G77_256EP_P10_ONE_ATTEMPT_ZERO_RETRY_INVENTORY_V1",
        "comparator_outcome_identity": "G77_256EP_E05_FIRST_AUTHORIZED_EQUAL_OUTCOME_001",
        "comparator_outcome": "EQUAL",
        "replay_context_identity": "G77_256EP_EXISTING_RUNTIMELEDGER_REPLAY_CONTEXT_V1",
    }
    ...
    payload = {
        ...
        "case_id": CASE_ID,
        "evidence_run_identity": "G77_256EP_CONSUMED_REUSE_EVIDENCE_RUN_001",
        ...
        "minimum_retention": "CD_AUTHORIZED_MINIMUM_TRAIL_PLUS_EN_RAW_PREIMAGE",
    }
    ...
    act = CanonicalHumanAuthorityActV1(
        ...
        interaction_identity="G77_256EP_INTERACTION_001",
        conversation_identity="G77_256EP_CONVERSATION_001",
        session_identity="G77_256EP_SESSION_001",
        actor_identity="HUMAN_CONSTITUTIONAL_AUTHORITY",
        request_identity="G77_256EP_REQUEST_001",
        continuation_identity="G77_256EP_CONTINUATION_001",
        ...
        metadata={
            ...
            "human_authorization_source": "G77_256EP_CURRENT_PROMPT",
    ...
    correlation = create_canonical_che_evidence_correlation_v1(
        ...
        source_channel_identity="G77_256EP_HUMAN_AUTHORIZATION_CHANNEL",
        adapter_identity="G77_256EP_CANONICAL_BINDING_ADAPTER",
        request_identity=act.request_identity,
        che_entry_identity="G77_256EP_CHE_ENTRY_001",
        source_act_identity=act.authority_act_identity,
        source_act_digest=replay_hash(act.to_dict()),
        order_identity="G77_256EP_ORDER_001",
        idempotency_identity="G77_256EP_IDEMPOTENCY_001",
        ...
        authority_result_identity="G77_256EP_AUTHORITY_RESULT_001",
        ...
        owner_projection_identity="G77_256EP_OWNER_PROJECTION_001",
        failure_identity=NOT_APPLICABLE,
        presentation_identity="G77_256EP_PRESENTATION_001",
        response_identity="G77_256EP_RESPONSE_001",
        response_digest=replay_hash({"response": "G77_256EP_AUTHORITY_RECORDED"}),
        delivery_record_identity="G77_256EP_DELIVERY_001",
...
def custody_process(
    ...
    try:
        ...
        request = CustodyRequest(
            ...
            request_identity="G77_256EP_CUSTODY_REQUEST_001",
        ...
        ledger_before_reuse_text = ledger_path.read_text(encoding="utf-8")
        ledger_entries_before_reuse = [
            json.loads(line) for line in ledger_before_reuse_text.splitlines()
        ]
        owner_revisions_before_reuse = [
            json.loads(path.read_text(encoding="utf-8"))
            for path in sorted(store.root.iterdir())
        ]
        send_message(control, {
            "message_type": "FIRST_EFFECT_COMPLETE",
            "first_output_canonical_utf8": output_bytes.decode("utf-8"),
            "first_output_record": json.loads(output_bytes),
            "owner_state_after_first_effect": {
                "state": current.state.value,
                "revision": current.revision,
                "output_record_identity": current.output_record_identity,
                "outcome": current.outcome,
            },
            "owner_revision_files_after_first_effect": owner_revisions_before_reuse,
            "runtime_ledger_entries_after_first_effect": ledger_entries_before_reuse,
            "runtime_ledger_canonical_utf8_after_first_effect": ledger_before_reuse_text,
        })
        if receive_message(reader)["command"] != "ATTEMPT_CONSUMED_REUSE":
            raise RuntimeError("consumed-reuse acknowledgement missing")
        reuse_connection, _ = server.accept()
        reuse_request = CustodyRequest(
            protocol_identity="P11_DA_DISPOSABLE_LOCAL_IPC_V1",
            operation=CustodyOperation.CLAIM_AND_INVOKE_ONCE,
            request_identity="G77_256EP_CUSTODY_REQUEST_CONSUMED_REUSE_002",
            canonical_payload=input_bytes,
        )
        reuse_error_type = None
        reuse_error = None
        second_output_bytes = None
        try:
            second_output_bytes = consumer.claim_and_invoke_once(
                reuse_connection,
                reuse_request,
                act=act,
                correlation=correlation,
            ...
            reuse_error_type = type(exc).__name__
            reuse_error = str(exc)
        finally:
            try:
                reuse_connection.sendall(b"1")
            except OSError:
                pass
            reuse_connection.close()
        after_reuse = store.current()
        owner_revisions_after_reuse = [
            json.loads(path.read_text(encoding="utf-8"))
            for path in sorted(store.root.iterdir())
        ]
        ledger_after_reuse_text = ledger_path.read_text(encoding="utf-8")
        ledger_entries_after_reuse = [
            json.loads(line) for line in ledger_after_reuse_text.splitlines()
        ]
        reuse_invariant_pass = all((
            second_output_bytes is None,
            reuse_error_type == "FailClosedRuntimeError",
            reuse_error == "one-use Human act is absent or no longer available",
            after_reuse.state.value == "CONSUMED",
            after_reuse.revision == 2,
            owner_revisions_after_reuse == owner_revisions_before_reuse,
            ledger_after_reuse_text == ledger_before_reuse_text,
            len(ledger_entries_after_reuse) == len(ledger_entries_before_reuse),
        ))
        send_message(control, {
            "message_type": "ATTEMPT_COMPLETE",
            "first_output_canonical_utf8": output_bytes.decode("utf-8"),
            "first_output_record": json.loads(output_bytes),
            "first_authorized_effect_count": 1,
            "reuse_request_identity": reuse_request.request_identity,
            "reuse_denial_error_type": reuse_error_type,
            "reuse_denial_error": reuse_error,
            "reuse_denial_point": "AFTER_D1_PEER_AND_PAYLOAD_AUTHENTICATION__BEFORE_SECOND_PRECLAIM_LEDGER_APPEND_AND_CLAIM",
            "second_output_present": second_output_bytes is not None,
            "second_protected_effect_count": 0 if second_output_bytes is None else 1,
            "second_invocation_count": 0 if second_output_bytes is None else 1,
            "owner_state_after_reuse": {
                "state": after_reuse.state.value,
                "revision": after_reuse.revision,
                "output_record_identity": after_reuse.output_record_identity,
                "outcome": after_reuse.outcome,
            },
            "owner_revision_files_before_reuse": owner_revisions_before_reuse,
            "owner_revision_files_after_reuse": owner_revisions_after_reuse,
            "runtime_ledger_entries_before_reuse": ledger_entries_before_reuse,
            "runtime_ledger_entries_after_reuse": ledger_entries_after_reuse,
            "runtime_ledger_canonical_utf8_after_reuse": ledger_after_reuse_text,
            "consumed_reuse_invariant_pass": reuse_invariant_pass,
        })
        reader.close()
        control.close()
        server.close()
...
def main() -> int:
    ...
    try:
        actual_harness = sha256_path(EN_HARNESS_PATH)
        if actual_harness != expected_harness:
            raise RuntimeError("EN harness hash mismatch")
        ...
        evidence_id = append_record("commissioning_P03", "EVIDENCE", {
            ...
            "dn_instrument_scope": "CURRENT_EN_ENVIRONMENT_PROSPECTIVE_ONLY",
        ...
        evidence_id = append_record("commissioning_P11", "HUMAN_DECISION", {
            "generation_authorization": "PRESENT__EXACT_EN_ONLY",
        ...
        preact = {
            "schema_id": "G77_256EP_PRE_ACT_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "SAME_LIVE_EN_GENERATION_ONLY__NO_REPLAY__NO_AUTHORITY_TRANSFER",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256EP_PRE_ACT_CHECKPOINT_V1",
                "sha256": preact_sha,
            },),
        )
        ...
        authority_seal = {
            "schema_id": "G77_256EP_AUTHORITY_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            "attempt_identity": ATTEMPT_ID,
            "authority_act_identity": ACT_ID,
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "EXACT_CURRENT_ACT_AVAILABLE_ONLY_INSIDE_SAME_LIVE_EN_GENERATION__NO_TRANSFER_OR_REPLAY",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256EP_AUTHORITY_CHECKPOINT_V1",
        ...
        append_record("p11_attempt_result", "FACT", {
            **result,
            ...
            "evidence_obligation_id": "P11-E05",
            "case_id": CASE_ID,
            "e05_consumed_negative_authority": {
                ...
                "reuse_denial_count": 1,
                "second_invocation_count": 0,
                "second_protected_effect_count": 0,
            },
            "execution_counters": counters,
            "result": "PASS__ONE_FIRST_AUTHORIZED_EFFECT__ACT_CONSUMED_AND_PERMANENTLY_EXHAUSTED__ONE_REUSE_ATTEMPT_DENIED_BEFORE_SECOND_PRECLAIM_LEDGER_APPEND__ZERO_SECOND_PROTECTED_EFFECT",
        })
        guest_seal = {
            "schema_id": "G77_256EP_GUEST_EXECUTION_SEAL_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [
                ...
                "ONE_CONSUMED_REUSE_ATTEMPT_DENIED_ZERO_SECOND_EFFECT",
            ],
            ...
            "evidence_obligation_id": "P11-E05",
            "case_id": CASE_ID,
            "operational_result": "PASS__CONSUMED_AUTHORITY_REUSE_DENIED__ONE_FIRST_EFFECT__ZERO_SECOND_EFFECT",
            "execution_counters": counters,
            "raw_evidence_prefix_sha256": sha256_path(RAW_PATH),
            "raw_record_count": SEQUENCE,
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "RESULT_RECONSTRUCTION_ONLY__NO_ADDITIONAL_ATTEMPT__NO_P12__NO_PRODUCTION",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            first_failure_or_current_result="PASS__E05_CONSUMED_REUSE_DENIED__ONE_FIRST_EFFECT__ZERO_SECOND_EFFECT",
            teardown_state="PENDING",
            authorized_next_action="TEARDOWN_AND_FINALIZATION_ONLY__NO_EXECUTION_REPLAY",
            additional_completed_seals=({
                "identity": "G77_256EP_GUEST_EXECUTION_SEAL_V1",
        ...
        teardown_seal = {
            "schema_id": "G77_256EP_GUEST_TEARDOWN_SEAL_V1",
            "generation_identity": GENERATION_ID,
        ...
        if first_failure is None and counters["e05_case_execution_count"] == 1:
            ...
            current_result = "PASS__E05_CONSUMED_REUSE_DENIAL__GUEST_TEARDOWN_COMPLETE"
        ...
        update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256EP_GUEST_TEARDOWN_SEAL_V1",
