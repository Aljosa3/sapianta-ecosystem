#!/usr/bin/env python3
"""Materialize the nonauthority G77-256JE FUTURE Human barrier.

The only executable path authenticates ratified JD, reuses the certified IF
...
JE_ROOT = ROOT / ".github/governance/evidence/g77_256je_future_fresh_human_authorized_operational_denial_v1"
LIVE = JE_ROOT / "live_binding"
OPERATION_ROOT = JE_ROOT / "operation_state"
TRANSIENT_ROOT = Path("/tmp/g77_256je_future_fresh_human_authorized_operational_denial_v1")
...
JD_ROOT = Path(".github/governance/evidence/g77_256jd_future_post_jc_live_binding_readiness_v1")
JD_REPORT = JD_ROOT / "G77_256JD_G48_IMPLEMENTATION_REPORT_V1.md"
JD_TERMINAL = JD_ROOT / "G77_256JD_SPCE_TERMINAL_REPOSITORY_ONLY_REDUCTION_V1.json"
JD_FORMALIZER = JD_ROOT / "analysis/G77_256JD_POST_JC_LIVE_BINDING_FORMALIZER_V1.py"
JD_TEST = JD_ROOT / "tests/test_g77_256jd_future_post_jc_live_binding_readiness_v1.py"
JD_HASHES = {
    JD_REPORT: "946538fcbd2ca0e413d7d65177f2de1f0dae4406a524417e3bb91e1f14a443c4",
    JD_TERMINAL: "6106f0028903f53f95a6befbfaf7a802b6ffaf76bc9a829e388d6b679746a30b",
    JD_FORMALIZER: "a0216ee7ac2cb908ce07bb967d8b4234447ff63b9cdb3fc6d6cd4f42ab2c83f4",
    JD_TEST: "ceff30790bcc2ff9e6f46a35491ac253673955581d6307a36c01372786493c38",
}
...
class JEBarrierError(ValueError):
    """One deterministic fail-closed JE barrier error."""
...
def load_module(relative: Path, name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise JEBarrierError(f"MODULE_UNAVAILABLE:{relative}")
...
FM = load_module(FM_PATH, "g77_256je_fm")
GN = load_module(GN_PATH, "g77_256je_gn")
GL = load_module(GL_PATH, "g77_256je_gl")
DU = load_module(DU_PATH, "g77_256je_du_v2")
EB = load_module(EB_PATH, "g77_256je_eb_v2")
EE = load_module(EE_PATH, "g77_256je_ee_v2")
...
def unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, item in pairs:
        if key in value:
            raise JEBarrierError(f"DUPLICATE_KEY:{key}")
        value[key] = item
    return value
...
def load_canonical(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or raw != canonical_bytes(value):
        raise JEBarrierError(f"NONCANONICAL_JSON:{path}")
    return value
...
def write_json(path: Path, value: dict[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        raise JEBarrierError(f"FRESH_ARTIFACT_COLLISION:{path}")
...
def authenticate_entry(remote_head: str, nested_remote_tag: str) -> dict[str, Any]:
    ...
    if any(observed[key] != expected for key, expected in expected.items()):
        raise JEBarrierError("EXACT_RATIFIED_JD_ENTRY_MISMATCH")
    for line in git("status", "--porcelain", "--untracked-files=all").splitlines():
        if not line.startswith("?? " + JE_ROOT.relative_to(ROOT).as_posix() + "/"):
            raise JEBarrierError("JE_BOUNDED_WORKTREE_SCOPE_VIOLATION")
    ...
    for label, revision in LINEAGE.items():
        if subprocess.run(
            ...
            raise JEBarrierError(f"LINEAGE_MISSING:{label}")
    ...
    if nested_state != {
        ...
        raise JEBarrierError("NESTED_AUTHORITY_MISMATCH")
    return observed | {"local_remote_equality": "VERIFIED", "lineage": lineage, "nested_authority": nested_state}


def reconstruct_jd() -> dict[str, Any]:
    identities: dict[str, Any] = {}
    for relative, expected_sha in JD_HASHES.items():
        committed = subprocess.check_output(["git", "show", f"{HEAD}:{relative}"], cwd=ROOT)
        current = (ROOT / relative).read_bytes()
        if committed != current or sha256_bytes(committed) != expected_sha:
            raise JEBarrierError(f"JD_COMMITTED_BYTE_MISMATCH:{relative}")
        identities[relative.name] = {
            "git_blob": git("rev-parse", f"{HEAD}:{relative}"),
            "sha256": expected_sha,
        }
    envelope = load_canonical(ROOT / JD_TERMINAL)
    if envelope["reduction_sha256"] != sha256_bytes(canonical_bytes(envelope["reduction"])):
        raise JEBarrierError("JD_INNER_SEAL_MISMATCH")
    reduction = envelope["reduction"]
    terminal = "A__FUTURE_POST_JC_COMMIT_LIVE_BINDING_AND_OPERATIONAL_READINESS_VERIFIED"
    if (
        reduction["terminal"] != terminal
        or reduction["mode"] != "POST_JC_COMMIT_LIVE_BINDING_STATIC_READINESS_ONLY__NO_AUTHORIZATION__NO_OPERATION"
        or reduction["du_eb_ee_v2"]["runtime_certification_role_separation"] != "VERIFIED__PRESERVED"
        or reduction["du_eb_ee_v2"]["former_worktree_drift_barrier"] != "VERIFIED__CLOSED_BY_COMMITTED_OWNER_BYTES"
        or reduction["reuse_impact"]["production_route_delta"] != "VERIFIED__0"
        or reduction["reuse"]["p11_mutation_count"] != "VERIFIED__0"
        or reduction["e05"]["after"] != "VERIFIED__10_OF_18"
    ):
        raise JEBarrierError("JD_TERMINAL_BOUNDARY_MISMATCH")
    return {
...
def reconstruct_ir() -> dict[str, Any]:
    ...
    for relative, expected_sha in IR_HASHES.items():
        ...
        current = (ROOT / relative).read_bytes()
        if committed != current or sha256_bytes(committed) != expected_sha:
            raise JEBarrierError(f"IR_COMMITTED_BYTE_MISMATCH:{relative}")
        identities[relative.name] = {
            "git_blob": git("rev-parse", f"{HEAD}:{relative}"),
            "sha256": expected_sha,
        }
...
def authenticate_future_semantics() -> dict[str, Any]:
    ...
    if envelope["binding_sha256"] != sha256_bytes(canonical_bytes(binding)):
        raise JEBarrierError("IF_ACT_CHE_SEAL_MISMATCH")
    ...
    if not result["evaluation"] < result["valid_from"] < result["valid_until"]:
        raise JEBarrierError("FUTURE_TIME_RELATION_MISMATCH")
    if (result["payload_digest"], result["source_act"], result["che_correlation"]) != (
        FUTURE_PAYLOAD, FUTURE_SOURCE_ACT, FUTURE_CHE,
    ):
        raise JEBarrierError("FUTURE_IDENTITY_MISMATCH")
    result.update({"future_semantic_mutation_count": 0, "wall_clock_dependency_count": 0})
    return result


def derive_identity() -> dict[str, Any]:
    expected_root = "g77_256je_future_fresh_human_authorized_operational_denial_v1"
    if JE_ROOT.name != expected_root:
        raise JEBarrierError("EVIDENCE_NAMESPACE_IDENTITY_MISMATCH")
    if FM.fresh_context.operation_vector(GENERATION) != VECTOR:
        raise JEBarrierError("CLOSED_VECTOR_IDENTITY_DERIVATION_FAILED")
    for identity in (GENERATION, OPERATION):
        ...
        if result.returncode not in (0, 1) or result.stdout:
            raise JEBarrierError(f"COMMITTED_IDENTITY_COLLISION:{identity}")
    return {
...
def materialize_v2_binding() -> dict[str, Any]:
    ...
    if sha256_path(source) != FUTURE_CANDIDATE_SHA:
        raise JEBarrierError("IF_CANDIDATE_IDENTITY_MISMATCH")
    ...
    readiness_candidate = readiness_root / "candidate/G77_256JE_V2_READINESS_CANDIDATE_V2.json"
    ...
    harness = bindings / "G77_256JE_EE_PATH_PROJECTION_FIXTURE_V1.py"
    eb_path = bindings / "G77_256JE_EB_RECEIPT_V2.json"
    ee_path = bindings / "G77_256JE_EE_RECEIPT_V2.json"
    for parent in (
    ...
    if set(du_result.values()) != {"PASS"} or eb_result["overall_result"] != "PASS":
        raise JEBarrierError("DU_EB_V2_BINDING_FAILED")
    if ee_result["pre_materialization_runtime_path_binding_result"] != "PASS":
        raise JEBarrierError("EE_V2_BINDING_FAILED")
    ...
    if (target["head"], target["tree"]) != (IF_HEAD, IF_TREE) or baseline != {"head": HEAD, "tree": TREE}:
        raise JEBarrierError("V2_RUNTIME_CERTIFICATION_ROLE_BINDING_FAILED")
    if ee["receipt"]["runtime_target_selection_binding"] != target or ee["receipt"]["certification_baseline"] != baseline:
        raise JEBarrierError("EB_EE_ROLE_DISAGREEMENT")
    return {
...
def materialize(args: argparse.Namespace) -> None:
    if LIVE.exists() or OPERATION_ROOT.exists() or TRANSIENT_ROOT.exists():
        raise JEBarrierError("JE_ONE_SHOT_NAMESPACE_NOT_FRESH")
    ...
    static = sealed(
        "G77_256JE_PREAUTHORITY_STATIC_READINESS_ENVELOPE_V1", "proof",
        {
            "schema_id": "G77_256JE_PREAUTHORITY_STATIC_READINESS_V1",
            "recorded_at_utc": recorded, "entry": entry,
            "jd_reconstruction": jd, "ir_reconstruction": ir,
    ...
    static_path = JE_ROOT / "G77_256JE_PREAUTHORITY_STATIC_READINESS_V1.json"
    write_json(static_path, static)

    observation = GL.prepare_and_observe_receipt_parent(ROOT, context)
    observation_path = JE_ROOT / "G77_256JE_GL_RECEIPT_PARENT_OBSERVATION_V1.json"
    ...
    equivalence = sealed(
        "G77_256JE_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256JE_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION, **gl_result},
    )
    equivalence_path = JE_ROOT / "G77_256JE_GL_PREAUTH_FINAL_ADMISSION_EQUIVALENCE_V1.json"
    write_json(equivalence_path, equivalence)

    counters = zero_counters()
    checkpoint_inner = {
        "schema_id": "G77_256JE_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1",
        ...
        "jd_terminal_reconstruction": jd, "ir_terminal_reconstruction": ir,
    ...
    checkpoint = sealed(
        "G77_256JE_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_ENVELOPE_V1",
        "checkpoint", checkpoint_inner,
    )
    checkpoint_path = JE_ROOT / "G77_256JE_PREAUTHORIZATION_SAFE_STOP_CHECKPOINT_V1.json"
    write_json(checkpoint_path, checkpoint)

    request_inner = {
        "schema_id": "G77_256JE_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1",
    ...
    request = sealed("G77_256JE_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_ENVELOPE_V1", "request", request_inner)
    request_path = JE_ROOT / "G77_256JE_HUMAN_OPERATIONAL_AUTHORIZATION_REQUEST_V1.json"
    write_json(request_path, request)
    presentation = GN.render_human_authorization_presentation(request_path)
    presentation_path = JE_ROOT / "G77_256JE_HUMAN_OPERATIONAL_AUTHORIZATION_PRESENTATION_V1.txt"
    if presentation_path.exists() or presentation_path.is_symlink():
        raise JEBarrierError("FRESH_PRESENTATION_COLLISION")
    ...
    gn_proof = sealed(
        "G77_256JE_GN_HUMAN_PRESENTATION_EQUIVALENCE_ENVELOPE_V1", "proof",
        {"schema_id": "G77_256JE_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1",
         "generation_identity": GENERATION, "operation_identity": OPERATION,
    ...
    write_json(JE_ROOT / "G77_256JE_GN_HUMAN_PRESENTATION_EQUIVALENCE_V1.json", gn_proof)

    counters["authorization_presentation"] = 1
    reduction = sealed(
        "G77_256JE_PREHUMAN_PHASE_A_REDUCTION_ENVELOPE_V1", "reduction",
        {
            "schema_id": "G77_256JE_PREHUMAN_PHASE_A_REDUCTION_V1",
            "recorded_at_utc": recorded, "generation_identity": GENERATION,
            "operation_identity": OPERATION,
            "terminal": "HUMAN_AUTHORIZATION_REQUIRED",
            "entry": entry, "jd_reconstruction": jd,
    ...
    write_json(JE_ROOT / "G77_256JE_PREHUMAN_PHASE_A_REDUCTION_V1.json", reduction)
