...
SCHEMA_ID = "G77_256DT_RAW_EXECUTION_EVIDENCE_V1"
GENERATION_ID = "G77_256DT_ONE_FRESH_BOUNDED_NON_PRODUCTION_E05_CONCURRENCY_P11_GENERATION_V1"
ATTEMPT_ID = "G77_256DT_E05_CONCURRENCY_ATTEMPT_001"
ACT_ID = "G77_256DT_EXACT_CURRENT_ONE_USE_HUMAN_OPERATIONAL_ACT_001"
CASE_ID = "G77_256DT_E05_TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS_EXACTLY_ONE_WINNER_001"
CHECKOUT = Path("/mnt/aigol")
RAW_ROOT = Path("/mnt/g77-evidence")
RAW_PATH = RAW_ROOT / "G77_256DT_RAW_EXECUTION_EVIDENCE_V1.jsonl"
DT_HARNESS_PATH = Path("/mnt/dp-harness/G77_256DT_P11_OPERATIONAL_HARNESS_V1.py")
...
PRE_ACT_SEAL_PATH = RAW_ROOT / "G77_256DT_PRE_ACT_CHECKPOINT_V1.json"
AUTHORITY_SEAL_PATH = RAW_ROOT / "G77_256DT_AUTHORITY_CHECKPOINT_V1.json"
GUEST_SEAL_PATH = RAW_ROOT / "G77_256DT_GUEST_EXECUTION_SEAL_V1.json"
TEARDOWN_SEAL_PATH = RAW_ROOT / "G77_256DT_GUEST_TEARDOWN_SEAL_V1.json"
CONTINUATION_MANIFEST_PATH = RAW_ROOT / "G77_256DT_CONTINUATION_MANIFEST_V1.json"
FIXTURE_ROOT = Path("/run/g77-256dt-p11")
...
def update_continuation_manifest(
    *,
    ...
    updated = {
        "schema_id": "G77_256DT_CONTINUATION_MANIFEST_ENVELOPE_V1",
        "manifest": manifest,
...
def create_input_and_authority(gate: Any, bindings: Any, store: Any) -> tuple[Any, ...]:
    ...
    contract_hash = replay_hash({"contract": "G77_256DT_E05_CONCURRENCY_FAIL_CLOSED_AUTHORITY_V1"})
    input_value = {
        ...
        "input_identity": "G77_256DT_E05_CONCURRENCY_INPUT_001",
        "provenance_identity": "G77_256DT_AUTHENTICATED_DP_CD_MINIMUM_PROVENANCE_V1",
        "contract_identity": "G77_256DT_E05_CONCURRENCY_FAIL_CLOSED_AUTHORITY_CONTRACT_V1",
        ...
        "p10_inventory_identity": "G77_256DT_P10_ONE_ATTEMPT_ZERO_RETRY_INVENTORY_V1",
        "comparator_outcome_identity": "G77_256DT_E05_CONCURRENCY_EQUAL_OUTCOME_001",
        "comparator_outcome": "EQUAL",
        "replay_context_identity": "G77_256DT_EXISTING_RUNTIMELEDGER_REPLAY_CONTEXT_V1",
    }
    ...
    payload = {
        ...
        "case_id": CASE_ID,
        "evidence_run_identity": "G77_256DT_EVIDENCE_RUN_001",
        ...
        "minimum_retention": "CD_AUTHORIZED_MINIMUM_TRAIL_PLUS_DT_RAW_PREIMAGE",
    }
    ...
    act = CanonicalHumanAuthorityActV1(
        ...
        interaction_identity="G77_256DT_INTERACTION_001",
        conversation_identity="G77_256DT_CONVERSATION_001",
        session_identity="G77_256DT_SESSION_001",
        actor_identity="HUMAN_CONSTITUTIONAL_AUTHORITY",
        request_identity="G77_256DT_REQUEST_001",
        continuation_identity="G77_256DT_CONTINUATION_001",
        ...
        metadata={
            ...
            "human_authorization_source": "G77_256DT_CURRENT_PROMPT",
    ...
    correlation = create_canonical_che_evidence_correlation_v1(
        ...
        source_channel_identity="G77_256DT_HUMAN_AUTHORIZATION_CHANNEL",
        adapter_identity="G77_256DT_CANONICAL_BINDING_ADAPTER",
        request_identity=act.request_identity,
        che_entry_identity="G77_256DT_CHE_ENTRY_001",
        source_act_identity=act.authority_act_identity,
        source_act_digest=replay_hash(act.to_dict()),
        order_identity="G77_256DT_ORDER_001",
        idempotency_identity="G77_256DT_IDEMPOTENCY_001",
        ...
        authority_result_identity="G77_256DT_AUTHORITY_RESULT_001",
        ...
        owner_projection_identity="G77_256DT_OWNER_PROJECTION_001",
        failure_identity=NOT_APPLICABLE,
        presentation_identity="G77_256DT_PRESENTATION_001",
        response_identity="G77_256DT_RESPONSE_001",
        response_digest=replay_hash({"response": "G77_256DT_AUTHORITY_RECORDED"}),
        delivery_record_identity="G77_256DT_DELIVERY_001",
...
def custody_process(
    ...
    try:
        ...
        for index, connection in enumerate(caller_connections, start=1):
            ...
            peer_facts.append({
                "contender_identity": f"G77_256DT_E05_CONTENDER_{index:03d}",
                "peer_pid": peer_pid,
                "peer_uid": peer_uid,
                "peer_gid": peer_gid,
        ...
        for index, connection in enumerate(caller_connections):
            ...
            if worker_pid == 0:
                ...
                request = CustodyRequest(
                    ...
                    request_identity=f"G77_256DT_CUSTODY_REQUEST_{index + 1:03d}",
...
def main() -> int:
    if len(sys.argv) != 6:
        raise SystemExit("expected DT_HARNESS_SHA SCHEMA_SHA HEAD TREE DN_HARNESS_SHA")
    ...
    try:
        actual_harness = sha256_path(DT_HARNESS_PATH)
        ...
        evidence_id = append_record("commissioning_P03", "EVIDENCE", {
            ...
            "dn_instrument_scope": "CURRENT_DT_ENVIRONMENT_PROSPECTIVE_ONLY",
        ...
        evidence_id = append_record("commissioning_P11", "HUMAN_DECISION", {
            "generation_authorization": "PRESENT__EXACT_DT_ONLY",
        ...
        preact = {
            "schema_id": "G77_256DT_PRE_ACT_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            ...
            "continuation": "SAME_LIVE_DT_GENERATION_ONLY__NO_REPLAY__NO_AUTHORITY_TRANSFER",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DT_PRE_ACT_CHECKPOINT_V1",
                "sha256": preact_sha,
            },),
        )
        ...
        authority_seal = {
            "schema_id": "G77_256DT_AUTHORITY_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            "attempt_identity": ATTEMPT_ID,
            "authority_act_identity": ACT_ID,
            ...
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "EXACT_CURRENT_ACT_AVAILABLE_ONLY_INSIDE_SAME_LIVE_DT_GENERATION__NO_TRANSFER_OR_REPLAY",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DT_AUTHORITY_CHECKPOINT_V1",
        ...
        guest_seal = {
            "schema_id": "G77_256DT_GUEST_EXECUTION_SEAL_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DT_GUEST_EXECUTION_SEAL_V1",
        ...
        teardown_seal = {
            "schema_id": "G77_256DT_GUEST_TEARDOWN_SEAL_V1",
            "generation_identity": GENERATION_ID,
        ...
        update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DT_GUEST_TEARDOWN_SEAL_V1",
