...
SCHEMA_ID = "G77_256DR_RAW_EXECUTION_EVIDENCE_V1"
GENERATION_ID = "G77_256DR_ONE_FRESH_BOUNDED_NON_PRODUCTION_E05_CONCURRENCY_P11_GENERATION_V1"
ATTEMPT_ID = "G77_256DR_E05_CONCURRENCY_ATTEMPT_001"
ACT_ID = "G77_256DR_EXACT_CURRENT_ONE_USE_HUMAN_OPERATIONAL_ACT_001"
CASE_ID = "G77_256DR_E05_TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS_EXACTLY_ONE_WINNER_001"
CHECKOUT = Path("/mnt/aigol")
RAW_ROOT = Path("/mnt/g77-evidence")
RAW_PATH = RAW_ROOT / "G77_256DR_RAW_EXECUTION_EVIDENCE_V1.jsonl"
DR_HARNESS_PATH = Path("/mnt/dp-harness/G77_256DR_P11_OPERATIONAL_HARNESS_V1.py")
...
PRE_ACT_SEAL_PATH = RAW_ROOT / "G77_256DR_PRE_ACT_CHECKPOINT_V1.json"
AUTHORITY_SEAL_PATH = RAW_ROOT / "G77_256DR_AUTHORITY_CHECKPOINT_V1.json"
GUEST_SEAL_PATH = RAW_ROOT / "G77_256DR_GUEST_EXECUTION_SEAL_V1.json"
TEARDOWN_SEAL_PATH = RAW_ROOT / "G77_256DR_GUEST_TEARDOWN_SEAL_V1.json"
CONTINUATION_MANIFEST_PATH = RAW_ROOT / "G77_256DR_CONTINUATION_MANIFEST_V1.json"
FIXTURE_ROOT = Path("/run/g77-256dr-p11")
...
def update_continuation_manifest(
    *,
    ...
    updated = {
        "schema_id": "G77_256DR_CONTINUATION_MANIFEST_ENVELOPE_V1",
        "manifest": manifest,
...
def create_input_and_authority(gate: Any, bindings: Any, store: Any) -> tuple[Any, ...]:
    ...
    contract_hash = replay_hash({"contract": "G77_256DR_E05_CONCURRENT_CLAIM_FAIL_CLOSED_AUTHORITY_V1"})
    input_value = {
        ...
        "input_identity": "G77_256DR_E05_CONCURRENCY_INPUT_001",
        "provenance_identity": "G77_256DR_AUTHENTICATED_DP_CD_MINIMUM_PROVENANCE_V1",
        "contract_identity": "G77_256DR_E05_CONCURRENT_CLAIM_FAIL_CLOSED_AUTHORITY_CONTRACT_V1",
        ...
        "p10_inventory_identity": "G77_256DR_P10_ONE_ATTEMPT_ZERO_RETRY_INVENTORY_V1",
        "comparator_outcome_identity": "G77_256DR_E05_CONCURRENCY_EQUAL_OUTCOME_001",
        "comparator_outcome": "EQUAL",
        "replay_context_identity": "G77_256DR_EXISTING_RUNTIMELEDGER_REPLAY_CONTEXT_V1",
    }
    ...
    payload = {
        ...
        "case_id": CASE_ID,
        "evidence_run_identity": "G77_256DR_EVIDENCE_RUN_001",
        ...
        "minimum_retention": "CD_AUTHORIZED_MINIMUM_TRAIL_PLUS_DR_RAW_PREIMAGE",
    }
    ...
    act = CanonicalHumanAuthorityActV1(
        ...
        interaction_identity="G77_256DR_INTERACTION_001",
        conversation_identity="G77_256DR_CONVERSATION_001",
        session_identity="G77_256DR_SESSION_001",
        actor_identity="HUMAN_CONSTITUTIONAL_AUTHORITY",
        request_identity="G77_256DR_REQUEST_001",
        continuation_identity="G77_256DR_CONTINUATION_001",
        ...
        metadata={
            ...
            "human_authorization_source": "G77_256DR_CURRENT_PROMPT",
    ...
    correlation = create_canonical_che_evidence_correlation_v1(
        ...
        source_channel_identity="G77_256DR_HUMAN_AUTHORIZATION_CHANNEL",
        adapter_identity="G77_256DR_CANONICAL_BINDING_ADAPTER",
        request_identity=act.request_identity,
        che_entry_identity="G77_256DR_CHE_ENTRY_001",
        source_act_identity=act.authority_act_identity,
        source_act_digest=replay_hash(act.to_dict()),
        order_identity="G77_256DR_ORDER_001",
        idempotency_identity="G77_256DR_IDEMPOTENCY_001",
        ...
        authority_result_identity="G77_256DR_AUTHORITY_RESULT_001",
        ...
        owner_projection_identity="G77_256DR_OWNER_PROJECTION_001",
        failure_identity=NOT_APPLICABLE,
        presentation_identity="G77_256DR_PRESENTATION_001",
        response_identity="G77_256DR_RESPONSE_001",
        response_digest=replay_hash({"response": "G77_256DR_AUTHORITY_RECORDED"}),
        delivery_record_identity="G77_256DR_DELIVERY_001",
...
def custody_process(
    ...
    try:
        ...
        if receive_message(reader)["command"] != "RUN_ONE_CONCURRENCY_CASE":
            raise RuntimeError("authority checkpoint acknowledgement missing")

        caller_connections = [server.accept()[0], server.accept()[0]]
        authenticated_peers: list[dict[str, Any]] = []
        for contender_index, connection in enumerate(caller_connections, 1):
            peer = read_kernel_peer_credentials(connection)
            role = consumer._peer_verifier.authenticate(
                CustodyOperation.CLAIM_AND_INVOKE_ONCE, peer
            )
            if role is not PrincipalRole.P11_ORCHESTRATION_CALLER_PRINCIPAL:
                raise RuntimeError("concurrent contender is not the fixed P11 caller")
            authenticated_peers.append({
                "contender_id": f"CONTENDER_{contender_index}",
                "pid": peer.pid,
                "uid": peer.uid,
                "gid": peer.gid,
                "authenticated_role": role.value,
            })
        if len({item["pid"] for item in authenticated_peers}) != 2:
            raise RuntimeError("concurrent contenders require two distinct caller processes")
        ...
        if binding != available.binding:
            raise RuntimeError("concurrency PRECLAIM differs from protected custody")
        consumer._append_operational_event(
            "P11_DA_OPERATIONAL_PRECLAIM",
            {
                ...
                "commissioning_gate_identity": gate.gate_identity,
                "authority_effect": 0,
                "production_routing_effect": 0,
            },
        )
        ...
        contender_outcomes: list[dict[str, Any]] = []

        def claim_contender(peer_facts: dict[str, Any]) -> None:
            try:
                barrier.wait(timeout=30)
                claimed_state = store.claim(
                    available,
                    claim_time_unix_ns=time.time_ns(),
                ...
                outcome = {
                    **peer_facts,
                    "claim_result": "WINNER",
                    "owner_state": claimed_state.state.value,
                    "owner_revision": claimed_state.revision,
                    "error_type": None,
                    "error": None,
                }
                ...
                outcome = {
                    **peer_facts,
                    "claim_result": "DENIED_FAIL_CLOSED",
                    "owner_state": None,
                    "owner_revision": None,
            ...
            with outcome_lock:
                contender_outcomes.append(outcome)

        threads = [
            threading.Thread(target=claim_contender, args=(peer,), daemon=False)
        ...
        if any(thread.is_alive() for thread in threads):
            raise RuntimeError("concurrent claim contender did not terminate")
        contender_outcomes.sort(key=lambda item: item["contender_id"])
        winners = [item for item in contender_outcomes if item["claim_result"] == "WINNER"]
        denials = [
            item for item in contender_outcomes
            if item["claim_result"] == "DENIED_FAIL_CLOSED"
        ]
        if len(winners) != 1 or len(denials) != 1:
            raise RuntimeError("concurrent claim cardinality is not exactly one winner")
        claimed = store.current()
        if claimed.state.value != "CLAIMED" or claimed.revision != 1:
            raise RuntimeError("concurrent claim linearization did not produce CLAIMED/1")
        ...
        send_message(control, {
            "message_type": "CONCURRENCY_CASE_COMPLETE",
            "selected_e05_case_class": "E05_CONCURRENCY",
            "selected_e05_case_id": CASE_ID,
            "claim_contenders": contender_outcomes,
            "claim_contender_count": 2,
            "winning_claim_count": 1,
            "denied_competing_claim_count": 1,
...
def main() -> int:
    if len(sys.argv) != 6:
        raise SystemExit("expected DR_HARNESS_SHA SCHEMA_SHA HEAD TREE DN_HARNESS_SHA")
    ...
    if any(path.exists() for path in expected_absent):
        raise SystemExit("DR evidence sink is not empty")
    if not CONTINUATION_MANIFEST_PATH.is_file():
        raise SystemExit("DR continuation manifest is absent")
    ...
    counters = {
        ...
        "concurrent_claim_contender_count": 0,
        "winning_claim_count": 0,
        "denied_competing_claim_count": 0,
        "p12_entry_count": 0,
        "production_route_count": 0,
    }
    ...
    try:
        actual_harness = sha256_path(DR_HARNESS_PATH)
        if actual_harness != expected_harness:
            raise RuntimeError("DR harness hash mismatch")
        ...
        evidence_id = append_record("commissioning_P03", "EVIDENCE", {
            ...
            "dn_instrument_scope": "CURRENT_DR_ENVIRONMENT_PROSPECTIVE_ONLY",
        ...
        evidence_id = append_record("commissioning_P11", "HUMAN_DECISION", {
            "generation_authorization": "PRESENT__EXACT_DR_ONE_CASE_ONLY",
        ...
        preact = {
            "schema_id": "G77_256DR_PRE_ACT_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [f"P{number:02d}" for number in range(1, 13)],
            "pending_gates": ["CREATE_ONE_USE_ACT", "SUBMIT", "TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS", "EXACTLY_ONE_WINNER", "ONE_INVOCATION", "TERMINAL_BIND", "PERMANENT_EXHAUSTION", "TEARDOWN"],
            ...
            "continuation": "SAME_LIVE_DR_GENERATION_ONLY__NO_REPLAY__NO_AUTHORITY_TRANSFER",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DR_PRE_ACT_CHECKPOINT_V1",
                "sha256": preact_sha,
            },),
        )
        ...
        authority_seal = {
            "schema_id": "G77_256DR_AUTHORITY_CHECKPOINT_V1",
            "generation_identity": GENERATION_ID,
            "attempt_identity": ATTEMPT_ID,
            "authority_act_identity": ACT_ID,
            ...
            "pending_gates": ["TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS", "EXACTLY_ONE_WINNER", "ONE_INVOCATION", "TERMINAL_BIND", "PERMANENT_EXHAUSTION", "TEARDOWN"],
            "execution_counters": counters,
            "first_failure": None,
            "teardown_state": "PENDING",
            "checkpoint_is_authority": False,
            "continuation": "EXACT_CURRENT_ACT_AVAILABLE_ONLY_INSIDE_SAME_LIVE_DR_GENERATION__NO_TRANSFER_OR_REPLAY",
        }
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            teardown_state="PENDING",
            authorized_next_action="SAME_LIVE_GENERATION_ONLY__RUN_ONE_TWO_CONTENDER_CONCURRENCY_CASE__FRESH_SESSION_MUST_NOT_RESUME_ACT",
            additional_completed_seals=({
                "identity": "G77_256DR_AUTHORITY_CHECKPOINT_V1",
        ...
        send_message(parent_control, {"command": "RUN_ONE_CONCURRENCY_CASE"})
        ...
        if result["message_type"] != "CONCURRENCY_CASE_COMPLETE":
        ...
        counters.update({
            ...
            "concurrent_claim_contender_count": 2,
            "winning_claim_count": 1,
            "denied_competing_claim_count": 1,
        })
        append_record("p11_attempt_result", "FACT", {
            **result,
            ...
            "execution_counters": counters,
            "result": "PASS__TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS__EXACTLY_ONE_WINNER__ONE_FAIL_CLOSED_DENIAL__ONE_EQUAL_ZERO_ROUTING_OUTPUT__ACT_CONSUMED",
        })
        guest_seal = {
            "schema_id": "G77_256DR_GUEST_EXECUTION_SEAL_V1",
            "generation_identity": GENERATION_ID,
            "source_head": expected_head,
            "source_tree": expected_tree,
            "completed_gates": [
                "P01-P12", "ACT_CREATE", "ACT_SUBMIT", "PRECLAIM",
                "TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS",
                "EXACTLY_ONE_WINNING_CLAIM", "ONE_FAIL_CLOSED_CLAIM_DENIAL",
            ...
            "selected_e05_case_class": "E05_CONCURRENCY",
            "selected_e05_case_id": CASE_ID,
            "operational_result": "PASS__TWO_AUTHENTICATED_CONCURRENT_CLAIM_CONTENDERS__EXACTLY_ONE_WINNER__ONE_FAIL_CLOSED_DENIAL__ONE_EQUAL_ZERO_ROUTING_OUTPUT__ACT_CONSUMED",
            "execution_counters": counters,
            "raw_evidence_prefix_sha256": sha256_path(RAW_PATH),
            "raw_record_count": SEQUENCE,
        ...
        manifest_file_sha, manifest_envelope = update_continuation_manifest(
            ...
            first_failure_or_current_result="PASS__E05_CONCURRENCY__TWO_CONTENDERS__EXACTLY_ONE_WINNER__ACT_CONSUMED",
            teardown_state="PENDING",
            authorized_next_action="TEARDOWN_AND_FINALIZATION_ONLY__NO_EXECUTION_REPLAY",
            additional_completed_seals=({
                "identity": "G77_256DR_GUEST_EXECUTION_SEAL_V1",
        ...
        teardown_seal = {
            "schema_id": "G77_256DR_GUEST_TEARDOWN_SEAL_V1",
            "generation_identity": GENERATION_ID,
        ...
        update_continuation_manifest(
            ...
            additional_completed_seals=({
                "identity": "G77_256DR_GUEST_TEARDOWN_SEAL_V1",
