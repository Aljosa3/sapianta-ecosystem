...
ENVELOPE_SCHEMA_ID = "SAPIANTA_SPCE_CONTINUATION_MANIFEST_ENVELOPE_V2"
MANIFEST_SCHEMA_ID = "SAPIANTA_SPCE_CONTINUATION_MANIFEST_V2"
SCHEMA_IDENTITY = "SAPIANTA_SPCE_CONTINUATION_MANIFEST_SCHEMA_V2"
...
SCHEMA_SHA256 = "09a9124bba387903ec80778e515cf87d8277de6effba8a5715c0c0b1d0d2d57f"
PRODUCER_IDENTITY = "G77_256DU_CANONICAL_MANIFEST_PRODUCER_V2"
CONSUMER_IDENTITY = "G77_256DU_PRE_MATERIALIZATION_CONSUMER_VALIDATOR_V2"
...
def validate_envelope(
    ...
    *,
    ...
    if frontier["requires_human_review"] is not True:
        _fail("FRONTIER_SEMANTICS_INVALID", "V2 requires Human review")
    ...
    if manifest["auto_continuable"] is not False:
        _fail("AUTO_CONTINUABLE_INCONSISTENT", "V2 is never auto-continuable")
...
def validate_file(
    ...
    *,
    ...
    if raw != canonical_bytes(envelope):
        _fail("CANONICAL_SERIALIZATION_INVALID", "manifest bytes are not canonical V2 JSON")
...
def build_du_fixture(repository_root: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "generation_identity": "G77_256DU_CANONICAL_CONTINUATION_MANIFEST_CONTRACT_V2",
        "required_head": head,
        "source_tree": tree,
        ...
        "frontier_state": {
            "constitutional_frontier": "DT_MANIFEST_PRODUCER_CONSUMER_SCHEMA_INCOMPATIBILITY_CLOSED_BY_DU_V2",
...
def run_self_test(repository_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    candidate = deepcopy(fixture)
    candidate["manifest"]["manifest_version"] = "1.0.0"
    _rehash(candidate)
    ...
    evidence = {
        "schema_id": "G77_256DU_COMPATIBILITY_VALIDATION_EVIDENCE_V2",
...
V1_VALIDATOR_SHA256 = "27457993a4e6b778cc65356cd9b17a1bf2665f4e6147608d27dc233ff512304d"
DISPATCH_FIELDS = frozenset({
    "family", "schema_identity", "version", "validator_identity", "producer_identity",
})
V1_CONTRACT_TUPLE = {
    "family": "DU",
    "schema_identity": "SAPIANTA_SPCE_CONTINUATION_MANIFEST_SCHEMA_V1",
    "version": "1.0.0",
    "validator_identity": "G77_256DU_PRE_MATERIALIZATION_CONSUMER_VALIDATOR_V1",
...
V2_CONTRACT_TUPLE = {
    ...
    "version": MANIFEST_VERSION,
    "validator_identity": CONSUMER_IDENTITY,
...
def dispatch_validate(
    ...
    *,
    ...
    """Dispatch only an exact governed DU V1 or V2 identity tuple."""
    if not isinstance(contract_tuple, dict) or set(contract_tuple) != DISPATCH_FIELDS:
        _fail("DISPATCH_TUPLE_INVALID", "DU dispatch requires one closed governed tuple")
    if contract_tuple == V2_CONTRACT_TUPLE:
        return validate_envelope(envelope, repository_root, expected_head=expected_head)
    if contract_tuple == V1_CONTRACT_TUPLE:
        ...
        if sha256_path(path) != V1_VALIDATOR_SHA256:
            _fail("V1_VALIDATOR_HASH_MISMATCH", "immutable DU V1 validator bytes differ")
        spec = importlib.util.spec_from_file_location("g77_256du_dispatch_v1", path)
        if spec is None or spec.loader is None:
            _fail("V1_VALIDATOR_IMPORT_FAILED", "immutable DU V1 validator could not load")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return module.validate_envelope(
            envelope, repository_root, expected_head=expected_head
        )
    _fail("DISPATCH_TUPLE_UNKNOWN", "unknown, mixed, downgraded, or cross-family DU tuple")
