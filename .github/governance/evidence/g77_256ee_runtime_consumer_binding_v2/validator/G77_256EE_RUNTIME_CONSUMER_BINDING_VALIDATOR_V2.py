#!/usr/bin/env python3
"""Bind one EB-validated Canonical V2 candidate to its exact runtime path."""
...
import jsonschema
ENVELOPE_SCHEMA_ID = "G77_256EE_RUNTIME_CONSUMER_BINDING_RECEIPT_ENVELOPE_V2"
RECEIPT_SCHEMA_ID = "G77_256EE_RUNTIME_CONSUMER_BINDING_RECEIPT_V2"
RECEIPT_VERSION = "2.0.0"
GENERATION_IDENTITY = "G77_256EE_REPOSITORY_ONLY_RUNTIME_CONSUMER_BINDING_HARDENING_V2"
VALIDATION_MODE = "PRE_MATERIALIZATION_RUNTIME_CONSUMER_BINDING"
VALIDATION_PROFILE = "DU_EB_CANONICAL_V2_RUNTIME_CONSUMER_BINDING_V2"
VALIDATOR_IDENTITY = "G77_256EE_RUNTIME_CONSUMER_BINDING_VALIDATOR_V2"
...
SCHEMA_IDENTITY = "G77_256EE_RUNTIME_CONSUMER_BINDING_RECEIPT_SCHEMA_V2"
SCHEMA_RELATIVE_PATH = (
    ".github/governance/evidence/g77_256ee_runtime_consumer_binding_v2/"
    "G77_256EE_RUNTIME_CONSUMER_BINDING_RECEIPT_SCHEMA_V2.json"
)
EB_VALIDATOR_IDENTITY = "G77_256EB_CANDIDATE_BOUND_PRE_MATERIALIZATION_VALIDATOR_V2"
...
def _authenticate_certification_baseline(repository_root: Path, baseline: Any) -> tuple[str, str]:
    if not isinstance(baseline, dict) or set(baseline) != {"head", "tree"}:
        _fail("CERTIFICATION_BASELINE_INVALID", "baseline must contain exactly head and tree")
    head, tree = baseline["head"], baseline["tree"]
    if GIT_OBJECT_RE.fullmatch(head or "") is None or GIT_OBJECT_RE.fullmatch(tree or "") is None:
        _fail("CERTIFICATION_BASELINE_FORMAT_INVALID", "baseline coordinates must be lowercase Git identities")
    try:
        head_tree = _git(repository_root, "rev-parse", f"{head}^{{tree}}")
    except BindingError as exc:
        raise BindingError("CERTIFICATION_BASELINE_COMMIT_NONEXISTENT", "baseline head is unavailable") from exc
    if head_tree != tree:
        _fail("CERTIFICATION_BASELINE_TREE_MISMATCH", "baseline tree does not belong to baseline head")
    if head != _git(repository_root, "rev-parse", "HEAD") or tree != _git(repository_root, "rev-parse", "HEAD^{tree}"):
        _fail("CERTIFICATION_BASELINE_STALE", "baseline is not the current certification repository")
    return head, tree
...
def validate_binding(
    ...
    """Validate and return a receipt bound to the current certification baseline."""
    ...
    try:
        ...
        raise BindingError(code, "candidate did not pass DU Canonical V2") from exc
...
def verify_receipt_envelope(
    ...
    certification_baseline = receipt["certification_baseline"]
    _authenticate_certification_baseline(repository_root, certification_baseline)
...
def run_self_test(
    ...
    with tempfile.TemporaryDirectory(prefix=".g77_256ee_selftest_", dir=repository_root) as raw_tmp:
        ...
        cases.append(_negative(
            ...
            "CERTIFICATION_BASELINE_COMMIT_NONEXISTENT",
            tampered(lambda value: value["receipt"]["certification_baseline"].update(
                {"head": "0" * 40}
            )),
        ))
        cases.append(_negative(
    ...
    return {
        "schema_id": "G77_256EE_RUNTIME_CONSUMER_BINDING_REGRESSION_EVIDENCE_V2",
        "generation_identity": GENERATION_IDENTITY,
        "certification_baseline": {
            "head": _git(repository_root, "rev-parse", "HEAD"),
            "tree": _git(repository_root, "rev-parse", "HEAD^{tree}"),
...
def run_current_self_test(
    ...
    """Exercise the current V2 positive path and certification firewalls."""
    positive = validate_binding(
        repository_root, candidate_path, eb_receipt_path, harness_path,
        runtime_export_root, guest_runtime_root,
    )
    verification = verify_receipt_envelope(repository_root, positive)
    cases: list[dict[str, str]] = []
    for case_id, mutation, expected in (
        ("BASELINE_NONEXISTENT", {"head": "0" * 40}, "CERTIFICATION_BASELINE_COMMIT_NONEXISTENT"),
        ("BASELINE_WRONG_TREE", {"tree": "0" * 40}, "CERTIFICATION_BASELINE_TREE_MISMATCH"),
        ("BASELINE_OPEN_OBJECT", {"branch": "FORBIDDEN"}, "CERTIFICATION_BASELINE_INVALID"),
    ):
        value = deepcopy(positive)
        value["receipt"]["certification_baseline"].update(mutation)
        _rehash_ee(value)
        try:
            verify_receipt_envelope(repository_root, value)
        except BindingError as exc:
            result = "PASS" if exc.code == expected else "FAIL"
            cases.append({"case_id": case_id, "expected": expected, "observed": exc.code, "result": result})
        else:
            cases.append({"case_id": case_id, "expected": expected, "observed": "NOT_REJECTED", "result": "FAIL"})
    return {
        "schema_id": "G77_256EE_RUNTIME_CONSUMER_BINDING_CURRENT_SELF_TEST_V2",
        "positive_result": verification["pre_materialization_runtime_path_binding_result"],
        "cases": cases,
        "overall_result": "PASS" if all(case["result"] == "PASS" for case in cases) else "FAIL",
...
def _failure(error: BindingError) -> bytes:
    return canonical_bytes({
        "schema_id": "G77_256EE_RUNTIME_CONSUMER_BINDING_FAILURE_V2",
...
V1_VALIDATOR_SHA256 = "5e4b35b3c7e7e23e5b7209c5f56e8a70055eac9a3deef32bc288b210e80f9410"
...
V1_CONTRACT_TUPLE = {
    ...
    "receipt_profile": "DU_EB_CANONICAL_V1_RUNTIME_CONSUMER_BINDING_V1",
    "issuer_implementation_identity": "G77_256EE_RUNTIME_CONSUMER_BINDING_VALIDATOR_V1",
}
V2_CONTRACT_TUPLE = {
    "family": "EE",
...
def dispatch_verify_receipt(
    ...
    """Dispatch only an exact governed EE V1 or V2 identity tuple."""
    if not isinstance(contract_tuple, dict) or set(contract_tuple) != DISPATCH_FIELDS:
        _fail("DISPATCH_TUPLE_INVALID", "EE dispatch requires one closed governed tuple")
    ...
    if contract_tuple == V1_CONTRACT_TUPLE:
        ...
        if sha256_path(path) != V1_VALIDATOR_SHA256:
            _fail("V1_VALIDATOR_HASH_MISMATCH", "immutable EE V1 validator bytes differ")
        spec = importlib.util.spec_from_file_location("g77_256ee_dispatch_v1", path)
        if spec is None or spec.loader is None:
            _fail("V1_VALIDATOR_IMPORT_FAILED", "immutable EE V1 validator could not load")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return module.verify_receipt_envelope(repository_root, envelope)
    _fail("DISPATCH_TUPLE_UNKNOWN", "unknown, mixed, downgraded, or cross-family EE tuple")
