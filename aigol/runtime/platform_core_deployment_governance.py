"""Governance-owned helpers for governed deployment operations."""

from __future__ import annotations

from pathlib import Path
from typing import Any

from aigol.authorization.authorization_record import create_authorization_record, validate_authorization_record
from aigol.runtime.models import FailClosedRuntimeError
from aigol.runtime.worker_runtime import AVAILABLE, register_worker
from aigol.workers.deployment_worker import (
    AUTHORIZED_DEPLOYMENT_REQUEST_TYPE,
    AUTHORIZED_DEPLOYMENT_SCOPE,
    DEPLOYMENT_WORKER_ID,
    DEPLOYMENT_WORKER_VERSION,
)


DEPLOYMENT_GOVERNANCE_VERSION = "G11_12_GOVERNED_DEPLOYMENT_WORKFLOW_IMPLEMENTATION_V1"


def create_governed_deployment_authorization_record(
    *,
    authorization_id: str,
    proposal_id: str,
    authorization_timestamp: str,
) -> dict[str, Any]:
    """Create a Governance authorization record for the Deployment Worker."""

    authorization = create_authorization_record(
        authorization_id=_require_string(authorization_id, "authorization_id"),
        proposal_id=_require_string(proposal_id, "proposal_id"),
        worker_id=DEPLOYMENT_WORKER_ID,
        authorization_scope=AUTHORIZED_DEPLOYMENT_SCOPE,
        authorization_timestamp=_require_string(authorization_timestamp, "authorization_timestamp"),
    ).to_dict()
    return validate_authorization_record(authorization)


def register_governed_deployment_worker(
    *,
    replay_dir: str | Path,
    created_at: str,
) -> dict[str, Any]:
    """Register the Deployment Worker through the Worker Runtime."""

    return register_worker(
        worker_id=DEPLOYMENT_WORKER_ID,
        worker_type="DEPLOYMENT_WORKER",
        worker_version=DEPLOYMENT_WORKER_VERSION,
        declared_capabilities=[
            "BOUNDED_DEPLOYMENT_OPERATION",
            "DEPLOYMENT_PLANNING",
            "DEPLOYMENT_EXECUTION",
            "DEPLOYMENT_VERIFICATION",
            "DEPLOYMENT_STATUS_REPORTING",
            "DEPLOYMENT_ROLLBACK_PREPARATION",
        ],
        supported_request_types=[AUTHORIZED_DEPLOYMENT_REQUEST_TYPE],
        trust_boundary="LOCAL_BOUNDED_WORKER",
        created_at=_require_string(created_at, "created_at"),
        replay_reference="replay:worker:governed-deployment:registration",
        replay_dir=replay_dir,
        state=AVAILABLE,
    )


def _require_string(value: Any, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise FailClosedRuntimeError(f"governed deployment requires {field}")
    return value
